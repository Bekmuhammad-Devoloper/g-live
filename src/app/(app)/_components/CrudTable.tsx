...
export default function CrudTable({ title, addLabel, rows, columns, fields, canManage, saveAction, deleteAction, searchable = true, locale = "uz" }: {
  ...
  return (
    <div className="space-y-4">
            ...
            <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
              {filtered.length === 0 ? (
                <tr><td colSpan={columns.length + 2} className="px-4 py-16 text-center">
                  ...
                  <p className="mt-0.5 text-xs text-slate-400">{tr(locale, { uz: "Filterni o'zgartirib ko'ring.", ru: "Попробуйте изменить фильтр.", en: "Try changing the filter.", de: "Versuchen Sie, den Filter zu ändern." })}</p>
                </td></tr>
              ) : filtered.map((row, i) => (
                  ...
                  {canManage && (
                    <td className="px-4 py-3">
                      <div className="flex items-center justify-end gap-2 text-slate-400">
                        <button onClick={() => openEdit(row)} className="transition hover:text-brand-600" title={tr(locale, { uz: "Tahrirlash", ru: "Редактировать", en: "Edit", de: "Bearbeiten" })}><Icon name="pencil" className="h-4 w-4" /></button>
                        <button onClick={() => del(String(row.id))} className="transition hover:text-rose-600" title={tr(locale, { uz: "O'chirish", ru: "Удалить", en: "Delete", de: "Löschen" })}><Icon name="trash" className="h-4 w-4" /></button>
                      </div>
                    </td>
                  )}
                </tr>
      ...
      {open && canManage && (
        <FormDrawer title={editing ? tr(locale, { uz: "Tahrirlash", ru: "Редактирование", en: "Edit", de: "Bearbeiten" }) : addLabel} fields={fields} editing={editing} locale={locale} saveAction={saveAction} onClose={() => setOpen(false)} onSaved={() => { setOpen(false); router.refresh(); }} />
      )}
    </div>
  );
}
...
function FormDrawer({ title, fields, editing, saveAction, onClose, onSaved, locale = "uz" }: {
  ...
  const submit = () => {
    ...
    start(async () => { const res = await saveAction(fd); if (res.ok) onSaved(); else setError(res.error ?? tr(locale, { uz: "Xatolik", ru: "Ошибка", en: "Error", de: "Fehler" })); });
  };
