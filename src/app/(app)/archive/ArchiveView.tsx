...
export default function ArchiveView({ locale, rows, reasons, roleOptions, canPurge }: { locale: Locale; rows: VArchived[]; reasons: string[]; roleOptions: RoleOpt[]; canPurge: boolean }) {
  ...
  const need = () => { if (sel.size === 0) { flash(tr(locale, { uz: "Avval belgilang", ru: "Сначала выберите", en: "Select first", de: "Zuerst auswählen" })); return false; } return true; };

  const onRestore = (ids: string[]) => start(async () => { const r = await restoreUsers(ids); if (r.ok) { setSel(new Set()); router.refresh(); flash(tr(locale, { uz: `${r.count} ta tiklandi`, ru: `Восстановлено: ${r.count}`, en: `${r.count} restored`, de: `${r.count} wiederhergestellt` })); } else flash(r.error ?? tr(locale, { uz: "Xatolik", ru: "Ошибка", en: "Error", de: "Fehler" })); });
  const onDelete = (ids: string[]) => { if (!confirm(tr(locale, { uz: `${ids.length} ta yozuvni butunlay o'chirasizmi? Bu qaytmas.`, ru: `Удалить ${ids.length} записей навсегда? Это необратимо.`, en: `Permanently delete ${ids.length} records? This cannot be undone.`, de: `${ids.length} Datensätze endgültig löschen? Dies kann nicht rückgängig gemacht werden.` }))) return; start(async () => { const r = await deleteUsersPermanent(ids); if (r.ok) { setSel(new Set()); router.refresh(); flash(tr(locale, { uz: `${r.count} o'chirildi${r.failed ? `, ${r.failed} o'chmadi (bog'liq ma'lumot yoki moliyaviy tarix — arxivda qoladi)` : ""}`, ru: `Удалено: ${r.count}${r.failed ? `, не удалено: ${r.failed} (связанные данные)` : ""}`, en: `${r.count} deleted${r.failed ? `, ${r.failed} not deleted (linked data)` : ""}`, de: `${r.count} gelöscht${r.failed ? `, ${r.failed} nicht gelöscht (verknüpfte Daten)` : ""}` })); } else flash(r.error ?? tr(locale, { uz: "Xatolik", ru: "Ошибка", en: "Error", de: "Fehler" })); }); };
  const onMessage = () => { if (!need()) return; const text = window.prompt(tr(locale, { uz: "Xabar matni:", ru: "Текст сообщения:", en: "Message text:", de: "Nachrichtentext:" }), ""); if (text == null) return; start(async () => { const r = await messageUsers(selIds(), text); flash(r.ok ? tr(locale, { uz: `${r.count} ga yuborildi`, ru: `Отправлено: ${r.count}`, en: `Sent to ${r.count}`, de: `An ${r.count} gesendet` }) : r.error ?? tr(locale, { uz: "Xatolik", ru: "Ошибка", en: "Error", de: "Fehler" })); }); };

  const exportCsv = () => exportRows("arxiv", [
    { key: "fullName", label: tr(locale, { uz: "Ism", ru: "Имя", en: "Name", de: "Name" }) }, { key: "phone", label: tr(locale, { uz: "Telefon", ru: "Телефон", en: "Phone", de: "Telefon" }) }, { key: "roleLabel", label: tr(locale, { uz: "Roli", ru: "Роль", en: "Role", de: "Rolle" }) },
    { key: "reason", label: tr(locale, { uz: "Sabab", ru: "Причина", en: "Reason", de: "Grund" }) }, { key: "note", label: tr(locale, { uz: "Izoh", ru: "Примечание", en: "Note", de: "Notiz" }) }, { key: "archivedAt", label: tr(locale, { uz: "Arxivlandi", ru: "Архивировано", en: "Archived", de: "Archiviert" }) },
