...
export const typeLabels = (locale: Locale): Record<string, string> => ({
  TASK: tr(locale, { uz: "Vazifa", ru: "Задание", en: "Task" }),
  EXAM: tr(locale, { uz: "Imtihon", ru: "Экзамен", en: "Exam" }),
  HOMEWORK: tr(locale, { uz: "Uy vazifasi", ru: "Домашнее задание", en: "Homework" }),
  TEST: tr(locale, { uz: "Test", ru: "Тест", en: "Test" }),
});
    ...
    <div className="fixed inset-0 z-[80]" onMouseDown={onClose}>
      ...
      >
        <div className="flex shrink-0 items-center justify-between border-b border-slate-100 px-5 py-4 dark:border-white/10">
          <h3 className="text-base font-bold text-slate-800 dark:text-slate-100">{tr(locale, { uz: "Imtihon / vazifa qo'shish", ru: "Добавить экзамен / задание", en: "Add exam / assignment" })}</h3>
        ...
        <div className="flex-1 space-y-3.5 overflow-y-auto px-5 py-4">
          <div>
            <label className={label}>{tr(locale, { uz: "Guruh", ru: "Группа", en: "Group" })} <span className="text-rose-500">*</span></label>
            <select name="groupId" required className={input} defaultValue="">
              <option value="" disabled>{tr(locale, { uz: "Guruhni tanlang", ru: "Выберите группу", en: "Select a group" })}</option>
              {groups.map((g) => <option key={g.id} value={g.id}>{g.name}</option>)}
            </select>
          </div>
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className={label}>{tr(locale, { uz: "Turi", ru: "Тип", en: "Type" })} <span className="text-rose-500">*</span></label>
              <select name="type" required className={input} defaultValue="TASK">
                {Object.entries(TYPE_LABELS).map(([v, l]) => <option key={v} value={v}>{l}</option>)}
              </select>
            </div>
            <div>
              <label className={label}>{tr(locale, { uz: "Maksimal ball", ru: "Максимальный балл", en: "Max score" })}</label>
          ...
          <div>
            ...
            <input name="title" required placeholder={tr(locale, { uz: "Masalan: A1 yakuniy imtihon", ru: "Например: итоговый экзамен A1", en: "e.g. A1 final exam" })} className={input} />
          </div>
          <div className="grid grid-cols-2 gap-3">
            <div>
            ...
            <div>
              <label className={label}>{tr(locale, { uz: "Topshirish muddati", ru: "Срок сдачи", en: "Due date" })}</label>
          ...
          {state.error && (
            <p className="rounded-lg bg-rose-50 px-3 py-2 text-sm text-rose-700 dark:bg-rose-500/10 dark:text-rose-400">
              {state.error === "forbidden" ? tr(locale, { uz: "Ruxsat yo'q.", ru: "Нет доступа.", en: "No permission." }) : tr(locale, { uz: "Ma'lumotlar to'liq emas.", ru: "Данные неполные.", en: "Incomplete data." })}
            </p>
          )}
        </div>
