...
import { FINANCE_HISTORY_ERROR, financeHistoryMessage, hasFinanceHistory, isRestrictError } from "@/lib/finance/guards";
...
export async function deleteBranch(id: string): Promise<{ ok?: boolean; error?: string }> {
  ...
  if (await hasFinanceHistory("branch", id)) return { error: FINANCE_HISTORY_ERROR };
  try {
...
export async function forceDeleteBranch(id: string, confirmName: string): Promise<{ ok?: boolean; error?: string; deleted?: PurgeSummary }> {
  ...
  // Finance V2: moliyaviy tarix (kassa, ledger, charge, V2 ga kiritilgan to'lov...) bo'lsa — to'lovlar hard-delete qilinmaydi (FINAL qoida)
  const postedPayments = await prisma.payment.count({ where: { student: { branchId: id }, postedAt: { not: null } } });
  if (postedPayments > 0 || (await hasFinanceHistory("branch", id))) return { error: financeHistoryMessage(s.locale) };
  ...
  await prisma.$transaction(async (tx) => {
    ...
    // Baza Restrict (Finance V2 qatorlari) — tranzaksiya to'liq qaytariladi, hech narsa o'chmaydi
    if (isRestrictError(e)) return { error: financeHistoryMessage(s.locale) };
    throw e;
  }
