...
export default async function SupportAnalyticsPage() {
  ...
  return (
    <>
      <PageHeader title={T("Qo'llab-quvvatlash analitikasi", "Аналитика поддержки", "Support analytics", "Support-Analytik")} subtitle={T(`Tizimdan foydalanish faolligi (oxirgi ${DAYS} kun, audit asosida)`, `Активность использования системы (последние ${DAYS} дн., по аудиту)`, `System usage activity (last ${DAYS} days, from audit)`)} />

      <div className="mb-6 grid grid-cols-2 gap-4 lg:grid-cols-3">
        <StatCard label={T("Jami amallar", "Всего действий", "Total actions", "Aktionen gesamt")} value={total} icon="history" />
        <StatCard label={T("Faol xodimlar", "Активных сотрудников", "Active staff", "Aktive Mitarbeiter")} value={actorIds.length} icon="user" />
        <StatCard label={T("Bekor qilishlar", "Отмены", "Cancellations", "Stornierungen")} value={cancels} tone={cancels ? "amber" : "green"} icon="eye" />
      </div>

      <div className="grid gap-5 lg:grid-cols-2">
        <Card>
          <h3 className="mb-4 text-sm font-semibold text-slate-700">{T("Bo'limlar bo'yicha amallar", "Действия по разделам", "Actions by module", "Aktionen nach Bereich")}</h3>
          {topEntities.length === 0 ? (
            <p className="py-8 text-center text-sm text-slate-400">{T("Bu davrda amallar yo'q", "За этот период действий нет", "No actions in this period", "Keine Aktionen in diesem Zeitraum")}</p>
          ) : (
        ...
        <Card padded={false}>
          <div className="border-b border-slate-100 px-5 py-4"><h3 className="text-sm font-semibold text-slate-700">{T("Eng faol xodimlar", "Самые активные сотрудники", "Most active staff", "Aktivste Mitarbeiter")}</h3></div>
          <Table head={<tr>
            <th className="px-4 py-3">{T("Xodim", "Сотрудник", "Staff", "Mitarbeiter")}</th>
            <th className="px-4 py-3">{T("Rol", "Роль", "Role", "Rolle")}</th>
            <th className="px-4 py-3 text-right">{T("Amallar", "Действия", "Actions", "Aktionen")}</th>
          </tr>}>
            {topActors.length === 0 ? (
              <EmptyRow colSpan={3} text={T("Bu davrda faollik yo'q", "За этот период активности нет", "No activity in this period", "Keine Aktivität in diesem Zeitraum")} />
            ) : topActors.map((a) => {
              const u = a.actorId ? actorMap.get(a.actorId) : null;
              return (
                <tr key={a.actorId ?? "system"} className="hover:bg-slate-50">
                  <td className="px-4 py-3 font-medium text-slate-800">{u?.fullName ?? T("Tizim", "Система", "System", "System")}</td>
