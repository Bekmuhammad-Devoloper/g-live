...
export default function CoursesTable({
  ...
  const STATUS_FILTERS: { value: string; label: string }[] = [
    { value: "ALL", label: tr(locale, { uz: "Barchasi", ru: "Все", en: "All", de: "Alle" }) },
    { value: "ACTIVE", label: tr(locale, { uz: "Faol", ru: "Активные", en: "Active", de: "Aktiv" }) },
    { value: "PLANNED", label: tr(locale, { uz: "Rejalashtirilgan", ru: "Запланировано", en: "Planned", de: "Geplant" }) },
    { value: "FINISHED", label: tr(locale, { uz: "Yakunlangan", ru: "Завершено", en: "Finished", de: "Abgeschlossen" }) },
    { value: "CANCELLED", label: tr(locale, { uz: "Bekor qilingan", ru: "Отменено", en: "Cancelled", de: "Storniert" }) },
  ];
  ...
  const lastColLabel = isOnline ? tr(locale, { uz: "Havola", ru: "Ссылка", en: "Link", de: "Link" }) : tr(locale, { uz: "Xona", ru: "Кабинет", en: "Room", de: "Raum" });

  return (
    <Card padded={false}>
      <div className="flex flex-wrap items-center justify-between gap-3 border-b border-slate-100 px-5 py-4 dark:border-slate-800">
        <h3 className="text-sm font-semibold text-slate-700">{isOnline ? tr(locale, { uz: "Onlayn guruhlar", ru: "Онлайн-группы", en: "Online groups", de: "Online-Gruppen" }) : tr(locale, { uz: "Offline guruhlar", ru: "Офлайн-группы", en: "Offline groups", de: "Offline-Gruppen" })}</h3>
        <div className="flex flex-wrap items-center gap-2">
          ...
          <Link href="/groups" className="flex items-center gap-1.5 rounded-lg border border-slate-200 px-3 py-1.5 text-xs font-semibold text-slate-600 transition hover:bg-slate-50 dark:border-slate-700 dark:text-slate-300 dark:hover:bg-slate-800">
            <Icon name="plus" className="h-3.5 w-3.5" /> {tr(locale, { uz: "Guruh qo'shish", ru: "Добавить группу", en: "Add group", de: "Gruppe hinzufügen" })}
          </Link>
        </div>
      </div>
      ...
      <Table
        head={
          <tr>
            ...
            <th className="px-4 py-3">{tr(locale, { uz: "Guruh nomi", ru: "Название группы", en: "Group name", de: "Gruppenname" })}</th>
            <th className="px-4 py-3">{tr(locale, { uz: "Kurs", ru: "Курс", en: "Course", de: "Kurs" })}</th>
            <th className="px-4 py-3">{tr(locale, { uz: "Daraja", ru: "Уровень", en: "Level", de: "Niveau" })}</th>
            <th className="px-4 py-3">{tr(locale, { uz: "Kun", ru: "Дни", en: "Days", de: "Tage" })}</th>
      ...
      >
        {filtered.length === 0 ? (
          <EmptyRow colSpan={9} text={isOnline ? tr(locale, { uz: "Onlayn guruh topilmadi", ru: "Онлайн-группы не найдены", en: "No online groups found", de: "Keine Online-Gruppen gefunden" }) : tr(locale, { uz: "Offline guruh topilmadi", ru: "Офлайн-группы не найдены", en: "No offline groups found", de: "Keine Offline-Gruppen gefunden" })} />
        ) : (
          filtered.map((g, i) => (
            <tr key={g.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/50">
              ...
              <td className="px-4 py-3">
                ...
                {g.isHybrid && <Badge tone="purple">{tr(locale, { uz: "Aralash", ru: "Смешанный", en: "Hybrid", de: "Hybrid" })}</Badge>}
              </td>
                ...
                {isOnline ? (
                  g.onlineLink ? (
                    <a href={g.onlineLink} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1 text-brand-600 hover:underline dark:text-brand-300">
                      <Icon name="video" className="h-3.5 w-3.5" /> {tr(locale, { uz: "Havola", ru: "Ссылка", en: "Link", de: "Link" })}
                    </a>
                  ) : (
      ...
      <div className="flex items-center justify-between gap-2 border-t border-slate-100 px-5 py-2.5 text-xs text-slate-400 dark:border-slate-800">
        <span>{tr(locale, { uz: "Ko'rsatilmoqda:", ru: "Показано:", en: "Showing:", de: "Angezeigt:" })} {filtered.length} / {rows.length}</span>
        <span className="flex items-center gap-1.5">
          {tr(locale, { uz: "Holat:", ru: "Статус:", en: "Status:", de: "Status:" })} <Badge tone={status === "ALL" ? "slate" : statusTone[status] ?? "slate"}>{STATUS_FILTERS.find((f) => f.value === status)?.label}</Badge>
        </span>
      </div>
    </Card>
  );
