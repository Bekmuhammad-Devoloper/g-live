...
export default function NewAssessmentForm({
  ...
  const SKILLS = [
    { v: "GENERAL", label: tr(locale, { uz: "Umumiy", ru: "Общее", en: "General", de: "Allgemein" }) },
    ...
    <div className="fixed inset-0 z-[80]" onMouseDown={onClose}>
      ...
      >
        <div className="flex shrink-0 items-center justify-between border-b border-slate-100 px-5 py-4 dark:border-white/10">
          <h3 className="text-base font-bold text-slate-800 dark:text-slate-100">{tr(locale, { uz: "Yangi mavsumiy baholash", ru: "Новое сезонное оценивание", en: "New seasonal assessment", de: "Neue saisonale Bewertung" })}</h3>
        ...
        <div className="flex-1 space-y-3.5 overflow-y-auto px-5 py-4">
          <div>
            ...
            <input name="title" required placeholder={tr(locale, { uz: "Qish mavsumi — oraliq baholash", ru: "Зимний сезон — промежуточное оценивание", en: "Winter season — interim assessment", de: "Wintersaison — Zwischenbewertung" })} className={input} />
          </div>
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className={label}>{tr(locale, { uz: "Mavsum", ru: "Сезон", en: "Season", de: "Saison" })} <span className="text-rose-500">*</span></label>
            ...
            <div>
              <label className={label}>{tr(locale, { uz: "Yil", ru: "Год", en: "Year", de: "Jahr" })} <span className="text-rose-500">*</span></label>
              <input name="year" type="number" min="2000" max="2100" required defaultValue={currentYear} className={input} />
            </div>
          </div>
          <div>
            <label className={label}>{tr(locale, { uz: "Guruh", ru: "Группа", en: "Group", de: "Gruppe" })}</label>
            <select name="groupId" className={input} defaultValue="">
              <option value="">{tr(locale, { uz: "Guruhsiz (umumiy)", ru: "Без группы (общее)", en: "No group (general)", de: "Ohne Gruppe (allgemein)" })}</option>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className={label}>{tr(locale, { uz: "Maksimal ball", ru: "Максимальный балл", en: "Maximum score", de: "Höchstpunktzahl" })}</label>
              <input name="maxScore" type="number" min="1" max="1000" defaultValue={100} className={input} />
            </div>
            <div>
              <label className={label}>{tr(locale, { uz: "O'tish balli", ru: "Проходной балл", en: "Passing score", de: "Bestehensgrenze" })}</label>
              <input name="passScore" type="number" min="0" max="1000" defaultValue={60} className={input} />
            </div>
          </div>
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className={label}>{tr(locale, { uz: "Sana", ru: "Дата", en: "Date", de: "Datum" })}</label>
              <input name="date" type="date" className={input} />
            </div>
            <div>
              <label className={label}>{tr(locale, { uz: "Holat", ru: "Статус", en: "Status", de: "Status" })}</label>
              <select name="status" className={input} defaultValue="PLANNED">
                <option value="PLANNED">{tr(locale, { uz: "Rejalashtirilgan", ru: "Запланировано", en: "Planned", de: "Geplant" })}</option>
                <option value="ONGOING">{tr(locale, { uz: "Jarayonda", ru: "В процессе", en: "Ongoing", de: "Laufend" })}</option>
                <option value="COMPLETED">{tr(locale, { uz: "Yakunlangan", ru: "Завершено", en: "Completed", de: "Abgeschlossen" })}</option>
              </select>
            </div>
          </div>
          <div>
            <label className={label}>{tr(locale, { uz: "Izoh", ru: "Комментарий", en: "Note", de: "Notiz" })}</label>
            <textarea name="note" rows={2} placeholder={tr(locale, { uz: "Qo'shimcha izoh", ru: "Дополнительный комментарий", en: "Additional note", de: "Zusätzliche Notiz" })} className="w-full rounded-lg border border-slate-200 bg-white px-3 py-2 text-sm text-slate-800 outline-none transition focus:border-brand-400 dark:border-slate-700 dark:bg-slate-800/60 dark:text-slate-100" />
          </div>

          {state.error && (
            <p className="rounded-lg bg-rose-50 px-3 py-2 text-sm text-rose-700 dark:bg-rose-500/10 dark:text-rose-400">
              {state.error === "forbidden" ? tr(locale, { uz: "Ruxsat yo'q.", ru: "Нет доступа.", en: "No permission.", de: "Keine Berechtigung." }) : tr(locale, { uz: "Ma'lumotlar to'liq emas.", ru: "Данные неполные.", en: "The data is incomplete.", de: "Die Daten sind unvollständig." })}
            </p>
          )}
        </div>
