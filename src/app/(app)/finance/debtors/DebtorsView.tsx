...
export default function DebtorsView({ debtors, groups, locale }: { debtors: VDebtor[]; groups: string[]; locale: Locale }) {
  ...
  const exportCsv = () => {
    const head = [
      tr(locale, { uz: "Talaba", ru: "Ученик", en: "Student" }),
      tr(locale, { uz: "Telefon", ru: "Телефон", en: "Phone" }),
      tr(locale, { uz: "Guruh", ru: "Группа", en: "Group" }),
      tr(locale, { uz: "Kurs", ru: "Курс", en: "Course" }),
      tr(locale, { uz: "Qarz miqdori", ru: "Сумма долга", en: "Debt amount" }),
      tr(locale, { uz: "Holati", ru: "Статус", en: "Status" }),
    ];
  ...
  return (
    <div className="space-y-5">
      <h1 className="flex items-baseline gap-3 text-[24px] font-bold tracking-tight text-slate-900 dark:text-slate-100">
        {tr(locale, { uz: "Qarzdorlar", ru: "Должники", en: "Debtors" })}
        <span className="text-sm font-normal text-slate-400">{tr(locale, { uz: "Miqdor", ru: "Количество", en: "Count" })} — {filtered.length}</span>
      </h1>
      ...
      <div className="rounded-2xl border border-l-4 border-slate-300 border-l-brand-600 bg-white p-5 shadow-card dark:border-slate-600 dark:border-l-brand-500 dark:bg-slate-900">
        <div className="flex items-center justify-between gap-3">
          <div className="text-xl text-slate-700 dark:text-slate-200">{tr(locale, { uz: "Jami", ru: "Итого", en: "Total" })}: <span className="font-bold text-slate-900 dark:text-slate-100">{formatMoney(total, locale)}</span></div>
      ...
      <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6">
        <FField label={tr(locale, { uz: "Qidiruv", ru: "Поиск", en: "Search" })}><input value={q} onChange={(e) => setQ(e.target.value)} placeholder={tr(locale, { uz: "Ism yoki telefon", ru: "Имя или телефон", en: "Name or phone" })} className={inp} /></FField>
        <FField label={tr(locale, { uz: "Talaba holati", ru: "Статус ученика", en: "Student status" })}>
          <Sel value={holati} onChange={setHolati} allowEmpty={false} locale={locale}
            options={[["FAOL", tr(locale, { uz: "Faol", ru: "Активные", en: "Active" })], ["ARXIV", tr(locale, { uz: "Arxivlangan", ru: "Архивные", en: "Archived" })], ["ALL", tr(locale, { uz: "Barchasi", ru: "Все", en: "All" })]]} />
        </FField>
        <FField label={tr(locale, { uz: "Guruh", ru: "Группа", en: "Group" })}><Sel value={group} onChange={setGroup} options={groups.map((g) => [g, g])} locale={locale} /></FField>
        <FField label={tr(locale, { uz: "Qarz miqdori (oldin)", ru: "Сумма долга (от)", en: "Debt amount (from)" })}><input value={debtMin} onChange={(e) => setDebtMin(e.target.value)} placeholder="min" className={inp} /></FField>
        <FField label={tr(locale, { uz: "Qarz miqdori (gacha)", ru: "Сумма долга (до)", en: "Debt amount (to)" })}><input value={debtMax} onChange={(e) => setDebtMax(e.target.value)} placeholder="max" className={inp} /></FField>
        ...
        <FField label={tr(locale, { uz: "Vazifa", ru: "Задача", en: "Task" })}><Sel value={vazifa} onChange={setVazifa} options={[["bor", tr(locale, { uz: "Vazifasi bor", ru: "С задачей", en: "Has task" })], ["yoq", tr(locale, { uz: "Vazifasi yo'q", ru: "Без задачи", en: "No task" })]]} locale={locale} /></FField>
        <div className="flex items-end">
          <button onClick={reset} className="h-11 rounded-full bg-brand-800 px-8 text-sm font-semibold text-white transition hover:bg-brand-900">{tr(locale, { uz: "Tozalash", ru: "Очистить", en: "Clear" })}</button>
        </div>
      </div>
      ...
      {filtered.length === 0 ? (
        <div className="rounded-2xl border border-emerald-200/60 bg-emerald-50/60 px-5 py-5 text-sm text-emerald-700 dark:border-emerald-900/40 dark:bg-emerald-950/20 dark:text-emerald-300">
          {tr(locale, { uz: "Ko'rsatiladigan ma'lumotlar yo'q", ru: "Нет данных для отображения", en: "No data to display" })}
        </div>
      ) : (
            ...
            <table className="w-full min-w-[860px] text-left text-sm">
              <thead className="border-b border-slate-200/70 bg-slate-50/60 text-[12px] font-semibold text-slate-600 dark:border-slate-800 dark:bg-slate-800/40 dark:text-slate-300">
                <tr>
                  ...
                  <th className="px-4 py-3.5">{tr(locale, { uz: "Qarz miqdori", ru: "Сумма долга", en: "Debt amount" })}</th>
                ...
                {filtered.map((d) => (
                  <tr key={d.studentId} className="transition hover:bg-slate-50 dark:hover:bg-slate-800/40">
                    <td className="px-4 py-3 font-medium text-slate-800 dark:text-slate-100">
                      {d.student}
                      {d.hasTask && <span className="ml-2 rounded bg-amber-100 px-1.5 py-0.5 text-[10px] font-semibold text-amber-700 dark:bg-amber-950/40 dark:text-amber-300">{tr(locale, { uz: "vazifa", ru: "задача", en: "task" })}</span>}
                    </td>
                    ...
                    <td className="px-4 py-3 text-right">
                      <Link href="/payments" className="inline-flex items-center gap-1.5 rounded-lg bg-brand-600 px-3 py-1.5 text-xs font-semibold text-white transition hover:bg-brand-700">
                        <Icon name="coins" className="h-3.5 w-3.5" /> {tr(locale, { uz: "To'lov", ru: "Платёж", en: "Payment" })}
                      </Link>
                    </td>
                  </tr>
                ))}
  ...
  return (
    <div className="relative">
      <select value={value} onChange={(e) => onChange(e.target.value)}
        ...
        {allowEmpty && <option value="">{tr(locale, { uz: "Tanlang", ru: "Выберите", en: "Select" })}</option>}
