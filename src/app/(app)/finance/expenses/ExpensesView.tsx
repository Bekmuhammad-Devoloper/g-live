...
const METHODS: { value: string; label: { uz: string; ru: string; en: string } }[] = [
  { value: "CASH", label: { uz: "Naqd pul", ru: "Наличные", en: "Cash" } },
  { value: "PAYME", label: { uz: "Payme", ru: "Payme", en: "Payme" } },
  { value: "CARD", label: { uz: "Plastik karta", ru: "Пластиковая карта", en: "Bank card" } },
  { value: "UZUM", label: { uz: "Uzum", ru: "Uzum", en: "Uzum" } },
  { value: "CLICK", label: { uz: "Click", ru: "Click", en: "Click" } },
  { value: "HUMO", label: { uz: "Humo", ru: "Humo", en: "Humo" } },
  { value: "BANK", label: { uz: "Bank hisobi", ru: "Банковский счёт", en: "Bank account" } },
];
...
export default function ExpensesView({ expenses, categories, staff, writable, locale, defaultFrom, defaultTo, today }: Props) {
  ...
  return (
    <div className="space-y-5">
      <h1 className="text-[24px] font-bold tracking-tight text-slate-900 dark:text-slate-100">{tr(locale, { uz: "Xarajatlar", ru: "Расходы", en: "Expenses" })}</h1>
        ...
        <div className="space-y-4">
          <div className="rounded-2xl border border-l-4 border-slate-300 border-l-brand-600 bg-white p-5 shadow-card dark:border-slate-600 dark:border-l-brand-500 dark:bg-slate-900">
            <div className="flex items-start justify-between gap-3">
              <div className="text-lg text-slate-700 dark:text-slate-200">
                {tr(locale, { uz: "Jami xarajatlar miqdori", ru: "Общая сумма расходов", en: "Total expenses amount" })}: <span className="font-bold text-slate-900 dark:text-slate-100">{formatMoney(total, locale)}</span>
              </div>
          ...
          <div className="rounded-2xl border border-slate-300 bg-white p-5 shadow-card dark:border-slate-600 dark:bg-slate-900">
            <div className="mb-3 flex items-center gap-2">
              ...
              <span className="text-sm text-slate-600 dark:text-slate-300">{tr(locale, { uz: "Xarajatlar", ru: "Расходы", en: "Expenses" })}</span>
            </div>
            {chart.length === 0 ? (
              <div className="flex h-52 items-center justify-center text-sm text-slate-400">{tr(locale, { uz: "Ko'rsatiladigan ma'lumotlar yo'q", ru: "Нет данных для отображения", en: "No data to display" })}</div>
            ) : (
      ...
      <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4 xl:grid-cols-7">
        <FField label={tr(locale, { uz: "Sanadan boshlab", ru: "С даты", en: "From date" })} req><input type="date" value={from} onChange={(e) => setFrom(e.target.value)} className={inp} /></FField>
        <FField label={tr(locale, { uz: "Sana bo'yicha", ru: "По дату", en: "To date" })} req><input type="date" value={to} onChange={(e) => setTo(e.target.value)} className={inp} /></FField>
        <FField label={tr(locale, { uz: "Nomi", ru: "Название", en: "Name" })}><input value={name} onChange={(e) => setName(e.target.value)} className={inp} /></FField>
        <FField label={tr(locale, { uz: "Kategoriyani tanlang", ru: "Выберите категорию", en: "Select category" })}><Sel value={cat} onChange={setCat} options={categories.map((c) => c.name)} locale={locale} /></FField>
        <FField label={tr(locale, { uz: "Oluvchi", ru: "Получатель", en: "Recipient" })}><input value={recipient} onChange={(e) => setRecipient(e.target.value)} className={inp} /></FField>
        <FField label={tr(locale, { uz: "To'lov turi", ru: "Тип платежа", en: "Payment type" })}><Sel value={method} onChange={setMethod} options={METHODS.map((m) => m.value)} labels={Object.fromEntries(METHODS.map((m) => [m.value, tr(locale, m.label)]))} locale={locale} /></FField>
        <div className="grid grid-cols-2 gap-2">
          <FField label={tr(locale, { uz: "Xodimni ismi", ru: "Имя сотрудника", en: "Staff name" })}><Sel value={staffF} onChange={setStaffF} options={staff} locale={locale} /></FField>
          <div className="flex items-end">
            <button onClick={() => { setFrom(defaultFrom); setTo(defaultTo); setName(""); setCat(""); setRecipient(""); setMethod(""); setStaffF(""); }}
              className="h-11 w-full rounded-full bg-brand-800 px-4 text-sm font-semibold text-white transition hover:bg-brand-900">{tr(locale, { uz: "Tozalash", ru: "Очистить", en: "Clear" })}</button>
          </div>
        </div>
      </div>
          ...
          <table className="w-full min-w-[900px] text-left text-sm">
            <thead className="border-b border-slate-200/70 bg-slate-50/60 text-[12px] font-semibold text-slate-600 dark:border-slate-800 dark:bg-slate-800/40 dark:text-slate-300">
              <tr>
                ...
                <th className="px-4 py-3.5">{tr(locale, { uz: "Turkum", ru: "Категория", en: "Category" })}</th>
                <th className="px-4 py-3.5">{tr(locale, { uz: "Nomi", ru: "Название", en: "Name" })}</th>
                <th className="px-4 py-3.5">{tr(locale, { uz: "Oluvchi", ru: "Получатель", en: "Recipient" })}</th>
                <th className="px-4 py-3.5">{tr(locale, { uz: "To'lov turi", ru: "Тип платежа", en: "Payment type" })}</th>
                ...
                <th className="px-4 py-3.5 text-right">{tr(locale, { uz: "Amallar", ru: "Действия", en: "Actions" })}</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
              {filtered.length === 0 ? (
                <tr><td colSpan={8} className="px-4 py-16 text-center text-slate-400">{tr(locale, { uz: "Ko'rsatiladigan ma'lumotlar yo'q", ru: "Нет данных для отображения", en: "No data to display" })}</td></tr>
              ) : (
                filtered.map((e) => (
...
function DeleteBtn({ id, locale }: { id: string; locale: Locale }) {
  ...
  const onClick = () => {
    if (!window.confirm(tr(locale, { uz: "Xarajatni o'chirishni tasdiqlaysizmi?", ru: "Подтвердить удаление расхода?", en: "Confirm deleting the expense?" }))) return;
    start(() => deleteExpense(id));
  };
  return (
    <button onClick={onClick} disabled={pending} title={tr(locale, { uz: "O'chirish", ru: "Удалить", en: "Delete" })}
      ...
      <Icon name="trash" className="h-3.5 w-3.5" /> {pending ? "..." : tr(locale, { uz: "O'chirish", ru: "Удалить", en: "Delete" })}
    </button>
  );
}
...
function ExportBtn({ rows, locale }: { rows: VExpense[]; locale: Locale }) {
  const onClick = () => {
    const head = [
      ...
      tr(locale, { uz: "Turkum", ru: "Категория", en: "Category" }),
      tr(locale, { uz: "Nomi", ru: "Название", en: "Name" }),
      tr(locale, { uz: "Oluvchi", ru: "Получатель", en: "Recipient" }),
      tr(locale, { uz: "To'lov turi", ru: "Тип платежа", en: "Payment type" }),
  ...
  return (
    <button onClick={onClick} title={tr(locale, { uz: "CSV yuklab olish", ru: "Скачать CSV", en: "Download CSV" })}
...
function AddCategory({ locale }: { locale: Locale }) {
  ...
  const onClick = () => {
    const name = window.prompt(tr(locale, { uz: "Yangi turkum nomi:", ru: "Название новой категории:", en: "New category name:" }));
    if (!name || name.trim().length < 2) return;
    setErr("");
    start(async () => {
      const res = await addExpenseCategory(name.trim());
      if (res.error === "duplicate") setErr(tr(locale, { uz: "Bunday turkum bor", ru: "Такая категория уже есть", en: "This category already exists" }));
      else router.refresh();
    });
  };
  return (
    <div className="flex flex-col items-end">
      <button onClick={onClick} disabled={pending}
        ...
        <Icon name="plus" className="h-4 w-4" /> {pending ? "..." : tr(locale, { uz: "Kategoriya qo'shing", ru: "Добавить категорию", en: "Add category" })}
      </button>
...
function NewExpense({ categories, today, locale }: { categories: { id: string; name: string }[]; today: string; locale: Locale }) {
  ...
  return (
    <form ref={formRef} action={action} className="h-fit rounded-2xl border border-slate-300 bg-white p-5 shadow-card dark:border-slate-600 dark:bg-slate-900">
      <h2 className="mb-4 text-xl font-bold text-slate-800 dark:text-slate-100">{tr(locale, { uz: "Yangi xarajatlar", ru: "Новый расход", en: "New expense" })}</h2>
      <input type="hidden" name="method" value={method} />

      <div className="space-y-3.5">
        <Lbl label={tr(locale, { uz: "Nomi", ru: "Название", en: "Name" })} req><input name="name" required className={fld} /></Lbl>
        <Lbl label={tr(locale, { uz: "Sana", ru: "Дата", en: "Date" })} req><input name="date" type="date" required defaultValue={today} className={fld} /></Lbl>
        <Lbl label={tr(locale, { uz: "Turkum", ru: "Категория", en: "Category" })}>
          <div className="relative">
        ...
        <Lbl label={tr(locale, { uz: "Oluvchi", ru: "Получатель", en: "Recipient" })}><input name="recipient" className={fld} /></Lbl>
        <Lbl label={tr(locale, { uz: "Sum", ru: "Сумма", en: "Amount" })} req><input name="amount" type="number" min="1" step="1000" required className={fld} /></Lbl>

        <div>
          <label className="mb-2 block text-sm font-medium text-slate-600 dark:text-slate-300">{tr(locale, { uz: "To'lov turi", ru: "Тип платежа", en: "Payment type" })} <span className="text-rose-500">*</span></label>
        ...
        {state.error && (
          <p className="rounded-lg bg-rose-50 px-3 py-2 text-sm text-rose-700 dark:bg-rose-950/30 dark:text-rose-300">
            {state.error === "forbidden" ? tr(locale, { uz: "Ruxsat yo'q.", ru: "Нет доступа.", en: "No permission." }) : tr(locale, { uz: "Majburiy maydonlarni to'ldiring.", ru: "Заполните обязательные поля.", en: "Fill in the required fields." })}
          </p>
        )}
        ...
        <button type="submit" disabled={pending}
          ...
          {pending ? "..." : tr(locale, { uz: "Saqlash", ru: "Сохранить", en: "Save" })}
        </button>
      </div>
    </form>
  );
