...
const SCOPES: { value: string; label: TR }[] = [
  { value: "TEACHER", label: { uz: "O'qituvchi bo'yicha", ru: "По преподавателю", en: "By teacher" } },
  { value: "COURSE", label: { uz: "Kurs bo'yicha", ru: "По курсу", en: "By course" } },
  { value: "GROUP", label: { uz: "Guruh bo'yicha", ru: "По группе", en: "By group" } },
  { value: "STUDENT", label: { uz: "Talaba bo'yicha", ru: "По ученику", en: "By student" } },
];
const scopeLabel: Record<string, TR> = {
  ALL: { uz: "Barcha o'qituvchilar", ru: "Все преподаватели", en: "All teachers" },
  TEACHER: { uz: "O'qituvchi bo'yicha", ru: "По преподавателю", en: "By teacher" },
  COURSE: { uz: "Kurs bo'yicha", ru: "По курсу", en: "By course" },
  GROUP: { uz: "Guruh bo'yicha", ru: "По группе", en: "By group" },
  STUDENT: { uz: "Talaba bo'yicha", ru: "По ученику", en: "By student" },
};
...
const typeLabel: Record<string, TR> = { FIXED: { uz: "O'zgarmas", ru: "Фиксированный", en: "Fixed" }, PERCENT: { uz: "Foiz", ru: "Процент", en: "Percent" } };
...
export default function SalaryCalculatorView({ rules, teachers, programs, groups, students, canManage, locale, period }: Props) {
  ...
  return (
    <div className="space-y-5">
      <h1 className="text-[24px] font-bold tracking-tight text-slate-900 dark:text-slate-100">{tr(locale, { uz: "Ish haqi", ru: "Зарплата", en: "Salary" })}</h1>
      ...
      <div className="overflow-hidden rounded-2xl border border-slate-300 bg-white shadow-card dark:border-slate-600 dark:bg-slate-900">
        ...
        <div className="flex items-center justify-between border-b border-slate-200/70 px-5 py-4 dark:border-slate-800">
          <button onClick={() => setCollapsed((v) => !v)} className="flex items-center gap-2.5 text-lg font-semibold text-slate-700 dark:text-slate-200">
            ...
            {tr(locale, { uz: "Ish haqi kalkulyatorini sozlash", ru: "Настройка калькулятора зарплаты", en: "Salary calculator settings" })}
        ...
        {!collapsed && (
          <div className="space-y-6 p-5">
            {/* 1-bosqich */}
            <Step n={1} text={tr(locale, { uz: "Barcha o'qituvchilar uchun standart xarajatlarni belgilash parametrlarini ko'rsating", ru: "Укажите параметры расчёта по умолчанию для всех преподавателей", en: "Specify the default calculation parameters for all teachers" })} />
            {canManage ? <DefaultRuleForm locale={locale} /> : <ReadOnlyNote locale={locale} />}

            {/* 2-bosqich */}
            <Step n={2} text={tr(locale, { uz: "Siz har qanday o'qituvchilar / kurslar / guruhlar / talabalar uchun individual hisob-kitobni belgilashingiz mumkin.", ru: "Вы можете задать индивидуальный расчёт для любых преподавателей / курсов / групп / учеников.", en: "You can set an individual calculation for any teachers / courses / groups / students." })} />
...
function ReadOnlyNote({ locale }: { locale: Locale }) {
  return <p className="text-sm text-slate-400">{tr(locale, { uz: "Faqat ko'rish rejimi — qoida qo'shish uchun ruxsat yo'q.", ru: "Режим только для просмотра — нет прав на добавление правил.", en: "View-only mode — no permission to add rules." })}</p>;
}
  ...
  return (
    <form ref={ref} action={action} className="flex flex-wrap items-end gap-3">
      ...
      <div className="min-w-[240px] flex-1">
        <label className="mb-1.5 block text-sm font-medium text-slate-500 dark:text-slate-400">{tr(locale, { uz: "Oylik miqdori", ru: "Сумма оклада", en: "Salary amount" })}</label>
        <div className="flex gap-2">
          ...
          <select name="amountType" defaultValue="FIXED" className={typeSel}>
            <option value="FIXED">{tr(locale, { uz: "O'zgarmas", ru: "Фиксированный", en: "Fixed" })}</option>
            <option value="PERCENT">{tr(locale, { uz: "Foiz", ru: "Процент", en: "Percent" })}</option>
          </select>
        </div>
      </div>
      <button type="submit" disabled={pending} className={addBtn}>{pending ? "..." : tr(locale, { uz: "Qo'shish", ru: "Добавить", en: "Add" })}</button>
      {state.error && <p className="w-full text-sm text-rose-500">{tr(locale, { uz: "Qiymatni to'g'ri kiriting.", ru: "Введите корректное значение.", en: "Enter a valid value." })}</p>}
    </form>
  );
}
  ...
  return (
    <form ref={ref} action={action} className="flex flex-wrap items-end gap-3">
      ...
      <div className="min-w-[200px]">
        <label className="mb-1.5 block text-sm font-medium text-slate-500 dark:text-slate-400">{tr(locale, { uz: "Hisoblash usuli", ru: "Способ расчёта", en: "Calculation method" })}</label>
        <div className="relative">
          <select name="scope" value={scope} onChange={(e) => { setScope(e.target.value); setTargetId(""); }} required
            ...
            <option value="">{tr(locale, { uz: "Tanlang", ru: "Выберите", en: "Select option" })}</option>
      ...
      <button type="submit" disabled={pending} className={addBtn}>{pending ? "..." : tr(locale, { uz: "Qo'shish", ru: "Добавить", en: "Add" })}</button>
      {state.error && <p className="w-full text-sm text-rose-500">{state.error === "target" ? tr(locale, { uz: "Nishonni tanlang.", ru: "Выберите цель.", en: "Select a target." }) : tr(locale, { uz: "Qiymatni to'g'ri kiriting.", ru: "Введите корректное значение.", en: "Enter a valid value." })}</p>}
    </form>
  );
}
    ...
    <div className="overflow-x-auto rounded-xl border border-slate-200/70 dark:border-slate-800">
      <table className="w-full min-w-[820px] text-left text-sm">
        <thead className="border-b border-slate-200/70 bg-slate-50/60 text-[12px] font-semibold text-slate-600 dark:border-slate-800 dark:bg-slate-800/40 dark:text-slate-300">
          <tr>
            <th className="px-4 py-3.5">{tr(locale, { uz: "Hisoblash usuli", ru: "Способ расчёта", en: "Calculation method" })}</th>
            <th className="px-4 py-3.5">{tr(locale, { uz: "Maosh turi", ru: "Тип оклада", en: "Salary type" })}</th>
            <th className="px-4 py-3.5">{tr(locale, { uz: "Miqdori", ru: "Сумма", en: "Amount" })}</th>
            <th className="px-4 py-3.5">{tr(locale, { uz: "Kurs", ru: "Курс", en: "Course" })}</th>
        ...
        <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
          {rules.length === 0 ? (
            <tr><td colSpan={7} className="px-4 py-12 text-center text-slate-400">{tr(locale, { uz: "Bo'sh", ru: "Пусто", en: "Empty" })}</td></tr>
          ) : (
            rules.map((r) => (
              <tr key={r.id} className={cn("transition hover:bg-slate-50 dark:hover:bg-slate-800/40", r.isDefault && "bg-brand-50/40 dark:bg-brand-950/10")}>
                ...
                <td className="px-4 py-3 text-slate-500">{r.scope === "TEACHER" ? r.targetName : r.scope === "STUDENT" ? `${tr(locale, { uz: "Talaba", ru: "Ученик", en: "Student" })}: ${r.targetName}` : r.scope === "ALL" ? tr(locale, { uz: "Barcha", ru: "Все", en: "All" }) : "—"}</td>
...
function DelRule({ id, locale }: { id: string; locale: Locale }) {
  ...
  return (
    <button onClick={() => { if (window.confirm(tr(locale, { uz: "Qoidani o'chirasizmi?", ru: "Удалить правило?", en: "Delete the rule?" }))) start(() => deleteSalaryRule(id)); }} disabled={pending}
      ...
      <form action={action} className="flex flex-wrap items-center gap-3">
        ...
        {canManage && (
          <button type="submit" disabled={pending} className="h-11 rounded-lg bg-brand-800 px-7 text-sm font-semibold text-white transition hover:bg-brand-900 disabled:opacity-50">
            {pending ? tr(locale, { uz: "Hisoblanmoqda...", ru: "Расчёт...", en: "Calculating..." }) : tr(locale, { uz: "Hisoblang", ru: "Рассчитать", en: "Calculate" })}
          </button>
        )}
        <Link href={`/salary?year=${y}&month=${Number(m)}`}
          ...
          {tr(locale, { uz: "O'qituvchilar profilida ko'rsatish", ru: "Показать в профилях преподавателей", en: "Show in teacher profiles" })}
        </Link>
      </form>
      {state.ok && (
        <p className="text-sm text-emerald-600 dark:text-emerald-400">
          ✓ {tr(locale, { uz: `${state.count} o'qituvchi uchun hisoblandi — jami ${formatMoney(state.total ?? 0, locale)}.`, ru: `Рассчитано для ${state.count} преподавателей — всего ${formatMoney(state.total ?? 0, locale)}.`, en: `Calculated for ${state.count} teachers — total ${formatMoney(state.total ?? 0, locale)}.` })}
        </p>
      )}
      {state.error && <p className="text-sm text-rose-500">{state.error === "invalid" ? tr(locale, { uz: "Davrni tanlang.", ru: "Выберите период.", en: "Select a period." }) : tr(locale, { uz: "Ruxsat yo'q.", ru: "Нет доступа.", en: "No permission." })}</p>}
    </div>
  );
}
