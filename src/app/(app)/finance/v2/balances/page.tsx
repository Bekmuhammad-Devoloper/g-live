...
export default async function BalancesV2Page({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  const all = (await studentBalancesReport(prisma, { branchId, onlyNonZero: true })).filter((r) => !q || r.fullName.toLowerCase().includes(q.toLowerCase()));
  const rows = all.slice(skip, skip + take);
  const totalDebt = all.reduce((a, r) => a + r.debt, 0);
  const totalCredit = all.reduce((a, r) => a + r.credit, 0);
  return (
    <>
      <PageHeader title={fin(L, "balances")} subtitle={`${fin(L, "debt")}: ${formatMoney(totalDebt, L)} · ${fin(L, "credit")}: ${formatMoney(totalCredit, L)}`} />
      <Card padded={false}>
        <ListControls locale={L} total={all.length} page={page} pageSize={PAGE_SIZE} q={q} />
        <Table head={<tr><th className="px-4 py-3 text-left">{fin(L, "student")}</th><th className="px-3 py-3 text-right">{fin(L, "debt")}</th><th className="px-3 py-3 text-right">{fin(L, "credit")}</th><th className="px-3 py-3 text-right">{fin(L, "net")}</th></tr>}>
          {rows.length === 0 ? <EmptyRow colSpan={4} text={fin(L, "empty")} /> : rows.map((r) => (
            <tr key={r.studentId} className="text-sm">
              <td className="px-4 py-2.5"><a className="font-medium text-brand-700 hover:underline" href={`/finance/v2/students/${r.studentId}`}>{r.fullName}</a></td>
