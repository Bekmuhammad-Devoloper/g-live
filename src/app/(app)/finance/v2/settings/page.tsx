...
export default async function FinanceSettingsPage({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  return (
    <>
      ...
      <SettingsView locale={L} enabled={flags.enabled} isDirector={session.role === ROLES.DIRECTOR} canReopen={can("FINANCE_PERIOD_REOPEN")} canRules={can("SALARY_RULE_MANAGE")} defaultFees={defaultFees} month={key} branches={branches} accountMap={map ?? JSON.stringify(PAYMENT_METHOD_TO_ACCOUNT_TYPE, null, 2)}
