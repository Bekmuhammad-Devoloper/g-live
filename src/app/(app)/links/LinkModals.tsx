...
export function DeleteConfirmModal({ locale, kind, label, onCancel, onConfirm }: {
  ...
  const title = kind === "link"
    ? tr(locale, { uz: "Linkni o'chirish", ru: "Удалить ссылку", en: "Delete link" })
    : tr(locale, { uz: "Vakansiyani o'chirish", ru: "Удалить вакансию", en: "Delete vacancy" });
  const body = kind === "link"
    ? tr(locale, { uz: "Bu linkni o'chirganingizdan so'ng, u orqali ariza topshirish imkonsiz bo'ladi.", ru: "После удаления этой ссылки подать заявку через неё будет невозможно.", en: "Once this link is deleted, applications can no longer be submitted through it." })
    : tr(locale, { uz: "Vakansiya va unga tegishli barcha linklar o'chiriladi. Ular orqali ariza topshirish imkonsiz bo'ladi.", ru: "Вакансия и все её ссылки будут удалены. Подать заявку через них будет невозможно.", en: "The vacancy and all of its links will be deleted. Applications can no longer be submitted through them." });
    ...
    <Shell title={title} onClose={onCancel}>
      <div className="space-y-4">
        <div className="flex items-start gap-3 rounded-xl border border-rose-500/20 bg-rose-500/10 p-4">
          ...
          <div>
            <p className="text-sm font-medium text-rose-700 dark:text-rose-300">{tr(locale, { uz: "Diqqat!", ru: "Внимание!", en: "Warning!" })}</p>
        ...
        <div className="flex justify-end gap-3">
          ...
          <button onClick={onConfirm} className="flex items-center gap-2 rounded-xl bg-rose-600 px-4 py-2.5 text-sm font-semibold text-white transition hover:bg-rose-700">
            <Icon name="trash" className="h-4 w-4" /> {tr(locale, { uz: "O'chirish", ru: "Удалить", en: "Delete" })}
          </button>
        </div>
      </div>
    </Shell>
...
export function ResultLinksModal({ locale, links, origin, onClose }: { locale: Locale; links: CreatedLink[]; origin: string; onClose: () => void }) {
  ...
  return (
    <Shell wide title={`✅ ${tr(locale, { uz: "Linklar yaratildi!", ru: "Ссылки созданы!", en: "Links created!" })}`} onClose={onClose}>
      <div className="space-y-4">
        <p className="text-sm text-slate-500 dark:text-slate-400">
          {links.length > 1
            ? tr(locale, { uz: `${links.length} ta platforma linki yaratildi. Har birini nusxalab kerakli platformaga joylashtiring:`, ru: `Создано ссылок: ${links.length}. Скопируйте каждую и разместите на нужной платформе:`, en: `${links.length} platform links created. Copy each one and place it on the right platform:` })
            : tr(locale, { uz: "Link yaratildi. Nusxalab kerakli joyga joylashtiring:", ru: "Ссылка создана. Скопируйте и разместите её:", en: "Link created. Copy it and place it where needed:" })}
        </p>
            ...
            return (
              <div key={cl.code} className="flex items-center justify-between gap-3 rounded-xl border border-slate-200/70 p-3 dark:border-slate-800" style={{ background: `${p.color}14` }}>
                ...
                >
                  ...
                  {isCopied ? tr(locale, { uz: "Nusxalandi", ru: "Скопировано", en: "Copied" }) : tr(locale, { uz: "Nusxalash", ru: "Копировать", en: "Copy" })}
                </button>
              </div>
            );
          })}
        ...
        <div className="flex justify-end pt-1">
          <button onClick={onClose} className="rounded-xl bg-brand-600 px-4 py-2.5 text-sm font-semibold text-white transition hover:bg-brand-700">{tr(locale, { uz: "Yopish", ru: "Закрыть", en: "Close" })}</button>
        </div>
      </div>
    </Shell>
  );
...
export function QrModal({ locale, code, onClose }: { locale: Locale; code: string; onClose: () => void }) {
  ...
  return (
    <Shell title={tr(locale, { uz: "QR kod", ru: "QR код", en: "QR code" })} onClose={onClose}>
      <div className="text-center">
        {err ? (
          <p className="py-8 text-sm text-rose-500">{err}</p>
        ) : !data ? (
          <div className="grid h-56 place-items-center text-sm text-slate-400">{tr(locale, { uz: "Yuklanmoqda...", ru: "Загрузка...", en: "Loading..." })}</div>
        ) : (
          <>
            ...
            <a href={data.qr} download={`qr-${code}.png`} className="mt-3 inline-flex items-center gap-1.5 rounded-xl bg-brand-600 px-4 py-2 text-sm font-semibold text-white transition hover:bg-brand-700">
              <Icon name="download" className="h-4 w-4" /> {tr(locale, { uz: "PNG yuklab olish", ru: "Скачать PNG", en: "Download PNG" })}
            </a>
          </>
        )}
      </div>
