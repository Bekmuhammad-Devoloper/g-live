...
export default function LinksTable({ rows, hasAny, expanded, onExpand, onCreate, ...c }: Common & {
  ...
  const heads = [
    { t: "#", a: "text-left" },
    { t: tr(locale, { uz: "Vakansiya", ru: "Вакансия", en: "Vacancy" }), a: "text-left" },
    { t: tr(locale, { uz: "Davlat", ru: "Страна", en: "Country" }), a: "text-left" },
    { t: tr(locale, { uz: "Link", ru: "Ссылка", en: "Link" }), a: "text-left" },
    { t: tr(locale, { uz: "Ko'rishlar", ru: "Просмотры", en: "Views" }), a: "text-center" },
    { t: tr(locale, { uz: "Arizalar", ru: "Заявки", en: "Applications" }), a: "text-center" },
    { t: tr(locale, { uz: "Status", ru: "Статус", en: "Status" }), a: "text-center" },
    { t: tr(locale, { uz: "Sana", ru: "Дата", en: "Date" }), a: "text-center" },
    { t: tr(locale, { uz: "Amallar", ru: "Действия", en: "Actions" }), a: "text-right" },
  ];
                  ...
                  <div className="flex flex-col items-center gap-3">
                    ...
                    <div>
                      <p className="font-medium text-slate-600 dark:text-slate-300">
                        {hasAny ? tr(locale, { uz: "Filtrlarga mos link yo'q", ru: "Нет ссылок по фильтрам", en: "No links match the filters" }) : tr(locale, { uz: "Havola topilmadi", ru: "Ссылки не найдены", en: "No links found" })}
                      </p>
                      <p className="mt-1 text-xs text-slate-400">
                        {hasAny ? tr(locale, { uz: "Filtrlarni o'zgartiring", ru: "Измените фильтры", en: "Change the filters" }) : tr(locale, { uz: "Birinchi vakansiya va linkni yarating", ru: "Создайте первую вакансию и ссылку", en: "Create your first vacancy and link" })}
                      </p>
                    </div>
                    {!hasAny && canManage && (
                      <button onClick={onCreate} className="flex items-center gap-1.5 rounded-xl bg-brand-600 px-4 py-2 text-xs font-semibold text-white transition hover:bg-brand-700">
                        <Icon name="plus" className="h-4 w-4" /> {tr(locale, { uz: "Yangi kurs + Link", ru: "Новый курс + Ссылка", en: "New course + Link" })}
                      </button>
                    )}
                  </div>
                </td>
      ...
      <tr className="transition-colors hover:bg-slate-50/70 dark:hover:bg-slate-800/40">
        ...
        <td className="px-5 py-4">
          {multi ? (
            <button onClick={onExpand} className="flex items-center gap-1.5 text-xs font-medium text-blue-500 transition hover:text-blue-600 dark:text-blue-400">
              ...
              {tr(locale, { uz: `${v.links.length} ta link`, ru: `${v.links.length} ссылок`, en: `${v.links.length} links` })}
            </button>
          ) : (
        ...
        <td className="px-5 py-4 text-center">
          {multi ? (
            <span className="inline-flex items-center gap-1.5 rounded-full bg-slate-100 px-2 py-0.5 text-[11px] font-semibold text-slate-500 dark:bg-slate-800 dark:text-slate-400">
              {v.activeLinks}/{v.links.length} {tr(locale, { uz: "faol", ru: "актив.", en: "active" })}
            </span>
          ) : (
        ...
        <td className="px-5 py-4">
          <div className="flex items-center justify-end gap-1">
            {multi ? (
              <button onClick={onExpand} title={open ? tr(locale, { uz: "Yig'ish", ru: "Свернуть", en: "Collapse" }) : tr(locale, { uz: "Ochish", ru: "Развернуть", en: "Expand" })} className="rounded-lg p-1.5 text-slate-400 transition hover:bg-blue-500/10 hover:text-blue-500">
            ...
            {canManage && (
              <button onClick={() => onDelete({ kind: "vacancy", id: v.id, label: v.title })} title={tr(locale, { uz: "O'chirish", ru: "Удалить", en: "Delete" })} className="rounded-lg p-1.5 text-slate-400 transition hover:bg-rose-500/10 hover:text-rose-500">
          ...
          <tr key={l.id} className="bg-slate-50/60 dark:bg-white/[0.02]">
            ...
            <td className="px-5 py-2.5">
              <div className="flex items-center justify-end gap-1">
                ...
                {canManage && (
                  <button onClick={() => onDelete({ kind: "link", id: l.id, label: `${p.label} · ${l.code}` })} title={tr(locale, { uz: "O'chirish", ru: "Удалить", en: "Delete" })} className="rounded-lg p-1 text-slate-400 transition hover:bg-rose-500/10 hover:text-rose-500">
...
function LinkActions({ link, size, locale, canManage, origin, onCopy, onQr, onRegen }: Common & { link: VLink; size: "sm" | "md" }) {
  ...
  return (
    <>
      <button onClick={() => onCopy(link.id, link.code)} title={tr(locale, { uz: "Link nusxalash", ru: "Копировать ссылку", en: "Copy link" })} className={cn("rounded-lg text-slate-400 transition hover:bg-blue-500/10 hover:text-blue-500", pad)}>
        <Icon name="copy" className={ic} />
      </button>
      <a href={`${origin}/apply/${link.code}`} target="_blank" rel="noopener noreferrer" title={tr(locale, { uz: "Linkni ochish", ru: "Открыть ссылку", en: "Open link" })} className={cn("rounded-lg text-slate-400 transition hover:bg-emerald-500/10 hover:text-emerald-500", pad)}>
        <Icon name="arrowUpRight" className={ic} />
      </a>
      <button onClick={() => onQr(link.code)} title={tr(locale, { uz: "QR kod", ru: "QR код", en: "QR code" })} className={cn("rounded-lg text-slate-400 transition hover:bg-slate-500/10 hover:text-slate-600 dark:hover:text-slate-200", pad)}>
        <Icon name="grid" className={ic} />
      </button>
      {canManage && (
        <button onClick={() => onRegen(link.id)} title={tr(locale, { uz: "Kodni yangilash", ru: "Обновить код", en: "Regenerate code" })} className={cn("rounded-lg text-slate-400 transition hover:bg-amber-500/10 hover:text-amber-500", pad)}>
...
function CodeChip({ locale, link, copied, onCopy, small }: { locale: Locale; link: VLink; copied: boolean; onCopy: (id: string, code: string) => void; small?: boolean }) {
  return (
    <div className="flex items-center gap-2">
      ...
      <button onClick={() => onCopy(link.id, link.code)} title={tr(locale, { uz: "Nusxa olish", ru: "Копировать", en: "Copy" })} className={cn("rounded-lg text-slate-400 transition hover:bg-blue-500/10 hover:text-blue-500", small ? "p-1" : "p-1.5")}>
...
function StatusToggle({ locale, link, canManage, onToggle, compact }: { locale: Locale; link: VLink; canManage: boolean; onToggle: (id: string) => void; compact?: boolean }) {
  if (link.expired) {
    return (
      <span className="inline-flex items-center gap-1 rounded-full bg-rose-50 px-2 py-0.5 text-[11px] font-semibold text-rose-600 dark:bg-rose-500/10 dark:text-rose-400">
        <span className="h-1.5 w-1.5 rounded-full bg-rose-500" /> {tr(locale, { uz: "Muddati o'tgan", ru: "Истёк", en: "Expired" })}
      </span>
    );
  }
  ...
  return (
    <button
      ...
      title={on ? tr(locale, { uz: "O'chirish", ru: "Выключить", en: "Turn off" }) : tr(locale, { uz: "Yoqish", ru: "Включить", en: "Turn on" })}
    ...
    >
      ...
      {!compact && (on ? tr(locale, { uz: "Faol", ru: "Активна", en: "Active" }) : tr(locale, { uz: "Nofaol", ru: "Неактивна", en: "Inactive" }))}
    </button>
  );
}
