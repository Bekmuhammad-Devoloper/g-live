...
export default function LinksView({ locale, vacancies, totals, platformStats, countries, vacancyOptions, courseOptions, canManage, preselectVacancyId }: {
  ...
  const copy = async (id: string, code: string) => {
    ...
    setToast({ msg: tr(locale, { uz: "Havola nusxa olindi", ru: "Ссылка скопирована", en: "Link copied" }), type: "ok" });
  };

  const onToggle = (id: string) => start(async () => {
    await toggleLink(id);
    router.refresh();
    setToast({ msg: tr(locale, { uz: "Link holati yangilandi", ru: "Статус ссылки обновлён", en: "Link status updated" }), type: "ok" });
  });

  const onRegen = (id: string) => start(async () => {
    await regenerateCode(id);
    router.refresh();
    setToast({ msg: tr(locale, { uz: "Yangi kod yaratildi", ru: "Новый код создан", en: "New code generated" }), type: "ok" });
  });
  ...
  const confirmDelete = () => {
    ...
    start(async () => {
      ...
      router.refresh();
      setToast({ msg: tr(locale, { uz: "O'chirildi", ru: "Удалено", en: "Deleted" }), type: "ok" });
    });
  };
  ...
  const exportCsv = () => {
    const rows = vacancies.flatMap((v) => v.links.map((l) => ({
      ...
      cvr: `${l.cvr}%`,
      status: l.expired
        ? tr(locale, { uz: "Muddati o'tgan", ru: "Истёк", en: "Expired" })
        : l.isActive
          ? tr(locale, { uz: "Faol", ru: "Активна", en: "Active" })
          : tr(locale, { uz: "Nofaol", ru: "Неактивна", en: "Inactive" }),
      date: l.createdAt,
    })));
    exportRows("havolalar", [
      { key: "code", label: tr(locale, { uz: "Kod", ru: "Код", en: "Code" }) },
      { key: "platform", label: tr(locale, { uz: "Platforma", ru: "Платформа", en: "Platform" }) },
      { key: "vacancy", label: tr(locale, { uz: "Vakansiya", ru: "Вакансия", en: "Vacancy" }) },
      { key: "company", label: tr(locale, { uz: "Kompaniya", ru: "Компания", en: "Company" }) },
      { key: "country", label: tr(locale, { uz: "Davlat", ru: "Страна", en: "Country" }) },
      { key: "views", label: tr(locale, { uz: "Ko'rishlar", ru: "Просмотры", en: "Views" }) },
      { key: "submissions", label: tr(locale, { uz: "Arizalar", ru: "Заявки", en: "Applications" }) },
      { key: "cvr", label: "CVR" },
      { key: "status", label: tr(locale, { uz: "Status", ru: "Статус", en: "Status" }) },
      { key: "date", label: tr(locale, { uz: "Sana", ru: "Дата", en: "Date" }) },
    ], rows);
  };

  const tAll = tr(locale, { uz: "Barcha davlatlar", ru: "Все страны", en: "All countries" });
  ...
  return (
    <div className="space-y-6 overflow-x-hidden">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <h1 className="text-xl font-semibold text-slate-900 dark:text-slate-100">{tr(locale, { uz: "Maxsus Linklar", ru: "Специальные ссылки", en: "Special Links" })}</h1>
          <p className="mt-0.5 text-sm text-slate-500 dark:text-slate-400">{tr(locale, { uz: "Vakansiya yaratish va unga maxsus link biriktirish", ru: "Создание вакансии и привязка к ней специальной ссылки", en: "Create a vacancy and attach a special link to it" })}</p>
        </div>
        <div className="flex flex-wrap items-center gap-2">
          <button onClick={exportCsv} className={btnOutline}>
            <Icon name="download" className="mr-2 h-4 w-4" /> {tr(locale, { uz: "Eksport", ru: "Экспорт", en: "Export" })}
          </button>
          {canManage && (
            <button onClick={() => setCreate("existing")} className={btnOutline}>
              <Icon name="link" className="mr-2 h-4 w-4" /> {tr(locale, { uz: "Mavjud vakansiyaga link", ru: "Ссылка к существующей вакансии", en: "Link to existing vacancy" })}
            </button>
          )}
          {canManage && (
            <button onClick={() => setCreate("new")} className="flex h-10 items-center rounded-xl bg-brand-600 px-4 text-sm font-semibold text-white shadow-sm transition hover:bg-brand-700">
              <Icon name="plus" className="mr-2 h-4 w-4" /> {tr(locale, { uz: "Yangi kurs + Link", ru: "Новый курс + Ссылка", en: "New course + Link" })}
            </button>
          )}
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-4">
        <Tile icon="link" tone="text-blue-500 dark:text-blue-400" label={tr(locale, { uz: "Jami linklar", ru: "Всего ссылок", en: "Total links" })} value={nf(totals.totalLinks)} />
        <Tile icon="check" tone="text-emerald-500 dark:text-emerald-400" label={tr(locale, { uz: "Faol", ru: "Активные", en: "Active" })} value={nf(totals.activeLinks)} />
        <Tile icon="eye" tone="text-purple-500 dark:text-purple-400" label={tr(locale, { uz: "Ko'rishlar", ru: "Просмотры", en: "Views" })} value={nf(totals.totalViews)} />
        <Tile icon="users" tone="text-amber-500 dark:text-amber-400" label={tr(locale, { uz: "Arizalar", ru: "Заявки", en: "Applications" })} value={nf(totals.totalSubmissions)} />
      </div>
        ...
        <div className="rounded-2xl border border-slate-200/70 bg-white p-5 shadow-card dark:border-slate-800 dark:bg-slate-900">
          <div className="mb-4 flex items-center justify-between">
            <h2 className="flex items-center gap-2 text-sm font-semibold text-slate-800 dark:text-slate-100">
              <Icon name="chart" className="h-4 w-4 text-slate-400" /> {tr(locale, { uz: "Platformalar statistikasi", ru: "Статистика платформ", en: "Platform statistics" })}
            </h2>
            {platFilter && (
              <button onClick={() => setPlatFilter(null)} className="rounded-lg bg-slate-100 px-2.5 py-1 text-xs font-medium text-slate-500 transition hover:bg-slate-200 dark:bg-slate-800 dark:text-slate-400 dark:hover:bg-slate-700">
                {tr(locale, { uz: "Barcha platformalar", ru: "Все платформы", en: "All platforms" })} ✕
              </button>
            )}
          </div>
            ...
            {platformStats.map((p) => (
              ...
              >
                ...
                <div className="space-y-1.5">
                  <PlatRow label={tr(locale, { uz: "Linklar", ru: "Ссылки", en: "Links" })} value={p.links} />
                  <PlatRow icon="eye" label={tr(locale, { uz: "Ko'rishlar", ru: "Просмотры", en: "Views" })} value={p.views} />
                  <PlatRow icon="users" label={tr(locale, { uz: "Arizalar", ru: "Заявки", en: "Applications" })} value={p.submissions} valueClass={p.submissions > 0 ? "text-emerald-600 dark:text-emerald-400" : "text-slate-400"} />
                  <div className="flex items-center justify-between border-t border-slate-200/70 pt-1 dark:border-slate-700/60">
                    <span className="text-[10px] text-slate-400">{tr(locale, { uz: "Ulush", ru: "Доля", en: "Share" })}</span>
      ...
      <div className="rounded-2xl border border-blue-500/20 bg-blue-500/10 p-4">
        <div className="flex items-start gap-3">
          ...
          <div>
            <p className="text-sm font-medium text-blue-700 dark:text-blue-300">{tr(locale, { uz: "Kurs + Link yaratish", ru: "Создание курса + ссылки", en: "Create course + link" })}</p>
            <p className="mt-1 text-xs text-blue-600/90 dark:text-blue-400">{tr(locale, {
              ...
              en: "Create a course link (name, level, price, description) and get a link automatically. Or add an extra link to an existing course. Each link leads to /apply/<code> — views and applications automatically become leads.",
            })}</p>
          </div>
        </div>
      </div>
      ...
      <div className="flex flex-wrap items-center gap-3">
        <div className="relative min-w-[220px] max-w-md flex-1">
          ...
          <input
            ...
            placeholder={tr(locale, { uz: "Link, vakansiya yoki davlat qidiring...", ru: "Поиск по ссылке, вакансии или стране...", en: "Search by link, vacancy or country..." })}
        ...
        <div className="relative" ref={dropRef}>
          ...
          {countryOpen && (
            <div className="absolute left-0 top-full z-50 mt-1 max-h-[280px] w-[220px] overflow-y-auto rounded-xl border border-slate-200 bg-white py-1 shadow-pop dark:border-slate-700 dark:bg-slate-900">
              ...
              {countries.length === 0 && <p className="px-4 py-2 text-xs text-slate-400">{tr(locale, { uz: "Davlat yo'q", ru: "Стран нет", en: "No countries" })}</p>}
            </div>
          )}
        </div>

        <button onClick={() => router.refresh()} title={tr(locale, { uz: "Yangilash", ru: "Обновить", en: "Refresh" })} aria-label={tr(locale, { uz: "Yangilash", ru: "Обновить", en: "Refresh" })} className="rounded-xl border border-slate-200 bg-white p-2.5 text-slate-500 transition hover:text-slate-800 dark:border-slate-700 dark:bg-slate-900 dark:hover:text-slate-100">
