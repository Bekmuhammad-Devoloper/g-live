...
export default function CancelButton({ id, locale }: { id: string; locale: Locale }) {
  ...
  function onClick() {
    const reason = window.prompt(tr(locale, { uz: "Bekor qilish sababi (majburiy, audit uchun):", ru: "Причина отмены (обязательно, для аудита):", en: "Cancellation reason (required, for audit):" }));
  ...
  return (
    ...
    >
      {pending ? "..." : tr(locale, { uz: "Bekor qilish", ru: "Отмена", en: "Cancel" })}
    </button>
  );
}
