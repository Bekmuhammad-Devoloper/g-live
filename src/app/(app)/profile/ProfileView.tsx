...
export default function ProfileView({ locale, me, stats }: {
  ...
  const pickPhoto = (f: File | null) => {
    if (!f) return;
    if (f.size > 900_000) { setError(tr(locale, { uz: "Rasm 900KB dan kichik bo'lsin", ru: "Фото должно быть меньше 900КБ", en: "Photo must be under 900KB" })); return; }
  ...
  const save = () => {
    ...
    start(async () => {
      ...
      if (r.ok) { flash(tr(locale, { uz: "Saqlandi ✓", ru: "Сохранено ✓", en: "Saved ✓" })); router.refresh(); }
  ...
  return (
    <div className="space-y-5">
        ...
        <div className="flex flex-wrap items-end gap-4 px-6 pb-5">
          <div className="-mt-12 shrink-0">
            <div className="relative">
              ...
              <button
                ...
                title={L("Rasm yuklash", "Загрузить фото", "Upload photo")}
          ...
          {img && (
            <button onClick={() => setImg(null)} className="rounded-lg border border-slate-200 px-3 py-1.5 text-xs font-medium text-slate-500 transition hover:bg-slate-50 dark:border-slate-700 dark:hover:bg-slate-800">
              {L("Rasmni o'chirish", "Удалить фото", "Remove photo")}
            </button>
          )}
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 gap-4 lg:grid-cols-4">
        <Tile icon="download" tone="#6366f1" label={L("Mening lidlarim", "Мои лиды", "My leads")} value={stats.leads} />
        <Tile icon="clipboard" tone="#f59e0b" label={L("Ochiq topshiriqlar", "Открытые задачи", "Open tasks")} value={stats.tasksOpen} />
        <Tile icon="phone" tone="#10b981" label={L("Qo'ng'iroqlarim", "Мои звонки", "My calls")} value={stats.calls} />
        <Tile icon="calendar" tone="#0ea5e9" label={L("Ro'yxatdan o'tgan", "Дата регистрации", "Joined")} value={me.joined} small />
      </div>

      {/* ── Tablar ── */}
      <div className="flex gap-2">
        <TabBtn active={tab === "info"} onClick={() => setTab("info")} icon="user">{L("Shaxsiy ma'lumotlar", "Личные данные", "Personal info")}</TabBtn>
        <TabBtn active={tab === "security"} onClick={() => setTab("security")} icon="shieldCheck">{L("Xavfsizlik", "Безопасность", "Security")}</TabBtn>
      </div>
      ...
      {tab === "info" ? (
        <div className="rounded-2xl border border-slate-200/70 bg-white p-6 shadow-card dark:border-slate-800 dark:bg-slate-900">
          <div className="grid gap-4 md:grid-cols-2">
            <Field label={L("F.I.Sh.", "Ф.И.О.", "Full name")} required>
              <input value={fullName} onChange={(e) => setFullName(e.target.value)} className={inp} />
            </Field>
            <Field label={L("Email (login uchun)", "Email (для входа)", "Email (for login)")} required>
              <input value={email} onChange={(e) => setEmail(e.target.value)} type="email" className={inp} />
            </Field>

            <Field label={L("Telefon", "Телефон", "Phone")}>
            ...
            <Field label={L("Tug'ilgan sana", "Дата рождения", "Date of birth")}>
              <input type="date" value={birthDate} onChange={(e) => setBirthDate(e.target.value)} className={inp} />
            </Field>

            <Field label={L("Jinsi", "Пол", "Gender")}>
              <div className="grid grid-cols-2 gap-2">
                <GBtn active={gender === "MALE"} onClick={() => setGender("MALE")} icon="♂" label={L("Erkak", "Мужчина", "Male")} color="#3b82f6" />
                <GBtn active={gender === "FEMALE"} onClick={() => setGender("FEMALE")} icon="♀" label={L("Ayol", "Женщина", "Female")} color="#ec4899" />
              </div>
            </Field>
            <Field label={L("Interfeys tili", "Язык интерфейса", "Interface language")}>
          ...
          <div className="mt-5 grid gap-3 rounded-xl bg-slate-50 p-4 text-sm dark:bg-white/[0.03] sm:grid-cols-3">
            <RO label={L("Lavozim", "Должность", "Position")} value={me.position ?? me.roleLabel} />
            <RO label={L("Filial", "Филиал", "Branch")} value={me.branchName ?? "—"} />
            <RO label={L("Oxirgi kirish", "Последний вход", "Last login")} value={me.lastLogin ?? "—"} />
          </div>
          <p className="mt-2 text-xs text-slate-400">{L("Rol va filialni faqat administrator o'zgartiradi.", "Роль и филиал меняет только администратор.", "Role and branch can only be changed by an administrator.")}</p>
        ...
        <div className="grid gap-4 lg:grid-cols-2">
          ...
          <div className="rounded-2xl border border-slate-200/70 bg-white p-6 shadow-card dark:border-slate-800 dark:bg-slate-900">
            <h3 className="mb-1 flex items-center gap-2 text-base font-semibold text-slate-800 dark:text-slate-100">
              <Icon name="eye" className="h-5 w-5 text-slate-400" /> {L("Hisob ma'lumotlari", "Данные аккаунта", "Account details")}
            </h3>
            <p className="mb-4 text-xs text-slate-400">{L("Hisobingiz holati va kirish tarixi.", "Состояние аккаунта и история входа.", "Your account status and sign-in history.")}</p>

            <div className="space-y-2.5">
              <InfoRow icon="mail" label={L("Email (login)", "Email (логин)", "Email (login)")} value={me.email} />
              <InfoRow icon="shieldCheck" label={L("Rol", "Роль", "Role")} value={me.roleLabel} />
              <InfoRow icon="building" label={L("Filial", "Филиал", "Branch")} value={me.branchName ?? "—"} />
              <InfoRow icon="clock" label={L("Oxirgi kirish", "Последний вход", "Last login")} value={me.lastLogin ?? "—"} />
              <InfoRow icon="calendar" label={L("Ro'yxatdan o'tgan", "Дата регистрации", "Joined")} value={me.joined} />
              {me.sipExtension && <InfoRow icon="phone" label={L("SIP raqami", "SIP номер", "SIP extension")} value={me.sipExtension} />}
            </div>
            ...
            <div className="mt-5 rounded-xl border border-amber-200 bg-amber-50 p-4 dark:border-amber-500/30 dark:bg-amber-500/10">
              <div className="flex items-center gap-2 text-sm font-semibold text-amber-700 dark:text-amber-400">
                <Icon name="info" className="h-4 w-4" /> {L("Xavfsizlik tavsiyalari", "Рекомендации по безопасности", "Security tips")}
              </div>
              <ul className="mt-2 space-y-1 text-xs text-amber-700/80 dark:text-amber-300/80">
                <li>• {L("Parolni hech kimga bermang.", "Никому не сообщайте пароль.", "Never share your password.")}</li>
                <li>• {L("Kamida 8 ta belgi, harf va raqam aralash bo'lsin.", "Минимум 8 символов, буквы и цифры.", "Use at least 8 characters, letters and numbers.")}</li>
                <li>• {L("Parolni har 3 oyda yangilab turing.", "Меняйте пароль каждые 3 месяца.", "Change your password every 3 months.")}</li>
                <li>• {L("Umumiy kompyuterda ishlagach, tizimdan chiqing.", "Выходите из системы на общих компьютерах.", "Sign out on shared computers.")}</li>
              </ul>
            </div>
          </div>
        </div>
...
function PasswordCard({ locale, onDone }: { locale: Locale; onDone: (m: string) => void }) {
  ...
  const submit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    start(async () => {
      ...
      if (r.ok) { ref.current?.reset(); onDone(L("Parol o'zgartirildi ✓", "Пароль изменён ✓", "Password changed ✓")); }
  ...
  return (
    <form ref={ref} onSubmit={submit} className="flex flex-col rounded-2xl border border-slate-200/70 bg-white p-6 shadow-card dark:border-slate-800 dark:bg-slate-900">
      <h3 className="mb-1 flex items-center gap-2 text-base font-semibold text-slate-800 dark:text-slate-100">
        <Icon name="shieldCheck" className="h-5 w-5 text-slate-400" /> {L("Parolni o'zgartirish", "Смена пароля", "Change password")}
      </h3>
      <p className="mb-4 text-xs text-slate-400">{L("Xavfsizlik uchun avval joriy parolni kiriting.", "Для безопасности сначала введите текущий пароль.", "For security, enter your current password first.")}</p>
      <div className="space-y-3">
        <Field label={L("Joriy parol", "Текущий пароль", "Current password")} required>
          <input name="current" type="password" required className={inp} />
        </Field>
        <Field label={L("Yangi parol", "Новый пароль", "New password")} required>
          <input name="next" type="password" required minLength={4} className={inp} />
        </Field>
        <Field label={L("Yangi parolni tasdiqlang", "Повторите новый пароль", "Confirm new password")} required>
      ...
      <button type="submit" disabled={pending} className="mt-auto w-full rounded-xl bg-brand-600 py-2.5 text-sm font-semibold text-white transition hover:bg-brand-700 disabled:opacity-60" style={{ marginTop: "1.25rem" }}>
        {pending ? L("O'zgartirilmoqda...", "Изменение...", "Changing...") : L("Parolni o'zgartirish", "Сменить пароль", "Change password")}
      </button>
    </form>
  );
}
