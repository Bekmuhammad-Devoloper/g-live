...
const COLS: { key: keyof Acc | "jamiAktiv" | "qarzFoiz"; label: Tr; pct?: boolean }[] = [
  { key: "buyurtma", label: { uz: "Buyurtma", ru: "Заявка", en: "Lead" } },
  { key: "trial", label: { uz: "Birinchi darsga keladiganlar", ru: "Приходящие на первый урок", en: "First-lesson attendees" } },
  { key: "yangi", label: { uz: "Yangi o'quvchi", ru: "Новые ученики", en: "New students" } },
  { key: "aktiv", label: { uz: "Aktiv o'quvchilar", ru: "Активные ученики", en: "Active students" } },
  { key: "realBor", label: { uz: "Jami real bor", ru: "Всего реально есть", en: "Total actually present" } },
  { key: "guruhOquvchi", label: { uz: "Guruh o'quvchilari", ru: "Ученики групп", en: "Group students" } },
  { key: "buyurtmaKetgan", label: { uz: "Buyurtmadan ketganlar", ru: "Ушедшие из заявки", en: "Left from lead" } },
  { key: "aktivKetgan", label: { uz: "Aktiv o'quvchidan ketganlar", ru: "Ушедшие активные ученики", en: "Left active students" } },
  { key: "qarzdor", label: { uz: "Qarzdorlar", ru: "Должники", en: "Debtors" } },
  { key: "guruh", label: { uz: "Guruh", ru: "Группа", en: "Group" } },
  { key: "birinchiTolov", label: { uz: "Birinchi to'lovni qilganlar", ru: "Совершившие первый платёж", en: "Made first payment" } },
  { key: "jamiOquvchi", label: { uz: "Jami o'quvchi", ru: "Всего учеников", en: "Total students" } },
  { key: "jamiAktiv", label: { uz: "Jami aktiv", ru: "Всего активных", en: "Total active" } },
  { key: "qarzFoiz", label: { uz: "Qarzdorlarning aktivga nisbatan foizi", ru: "Доля должников от активных", en: "Debtors as % of active" }, pct: true },
];
...
export default async function BranchesStatusPage({ searchParams }: { searchParams: Promise<Record<string, string | undefined>> }) {
  ...
  if (noneAcc.jamiOquvchi > 0 || noneAcc.buyurtma > 0 || noneAcc.guruh > 0) rows.push({ id: "none", name: tr(s.locale, { uz: "Filialsiz", ru: "Без филиала", en: "No branch" }), ...noneAcc });
  ...
  const exportColumns = [{ key: "name", label: tr(s.locale, { uz: "Filial", ru: "Филиал", en: "Branch" }) }, ...COLS.map((c) => ({ key: String(c.key), label: tr(s.locale, c.label) }))];
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <h1 className="text-[22px] font-bold tracking-tight text-slate-900 dark:text-slate-100">{tr(s.locale, { uz: "Filiallar holati", ru: "Состояние филиалов", en: "Branches status" })}</h1>
        ...
        <div className="overflow-x-auto">
          <table className="w-full min-w-[1900px] text-left text-sm">
            <thead className="border-b border-slate-200/70 bg-slate-50/80 text-[10px] font-semibold uppercase tracking-wide text-slate-500 dark:border-slate-800 dark:bg-slate-800/50">
              <tr>
                ...
                <th className="sticky left-12 z-10 min-w-[150px] bg-slate-50 px-4 py-3 dark:bg-slate-800">
                  <SortHeader col="name" label={tr(s.locale, { uz: "Filial", ru: "Филиал", en: "Branch" })} active={sortKey} dir={dir} locale={s.locale} />
                </th>
                {COLS.map((c) => (
            ...
            <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
              {rows.length === 0 ? (
                <tr><td colSpan={COLS.length + 2} className="py-16 text-center text-slate-400">{tr(s.locale, { uz: "Ma'lumotlar topilmadi", ru: "Данные не найдены", en: "No data found" })}</td></tr>
              ) : rows.map((r, i) => (
            ...
            {rows.length > 0 && (
              <tfoot className="border-t-2 border-slate-200 bg-slate-50/70 text-sm font-bold text-slate-700 dark:border-slate-700 dark:bg-slate-800/50 dark:text-slate-200">
                <tr>
                  ...
                  <td className="sticky left-12 z-10 bg-slate-50 px-4 py-3 dark:bg-slate-800">{tr(s.locale, { uz: "Umumiy natija", ru: "Итого", en: "Total" })}</td>
                  {COLS.map((c) => {
