  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <h1 className="text-[22px] font-bold tracking-tight text-slate-900 dark:text-slate-100">{tr(locale, { uz: "Davomati bekor qilinganlar analitikasi", ru: "Аналитика отменённой посещаемости", en: "Cancelled attendance analytics", de: "Analyse der stornierten Anwesenheit" })}</h1>
        <div className="flex flex-wrap items-center gap-2">
          <div className="relative">
            <Icon name="search" className="pointer-events-none absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-slate-400" />
            <input value={search} onChange={(e) => { setSearch(e.target.value); setPage(1); }} placeholder={tr(locale, { uz: "Qidiruv", ru: "Поиск", en: "Search", de: "Suchen" })} className="h-11 w-48 rounded-xl border border-slate-200 bg-white pl-9 pr-3 text-sm outline-none focus:border-brand-400 dark:border-slate-700 dark:bg-slate-800 dark:text-slate-100" />
          </div>
          ...
          <button
            onClick={() => exportRows("davomat-bekor-qilinganlar", [
              ...
              { key: "cancelledBy", label: tr(locale, { uz: "Kim bekor qildi", ru: "Кто отменил", en: "Cancelled by", de: "Storniert von" }) },
        ...
        <div className="overflow-x-auto">
          <table className="w-full min-w-[820px] text-left text-sm">
            <thead className="border-b border-slate-200/70 bg-slate-50/80 text-[11px] font-semibold uppercase tracking-wider text-slate-500 dark:border-slate-800 dark:bg-slate-800/50">
              <tr>
                <th className="w-12 px-4 py-3">№</th><th className="px-4 py-3">{tr(locale, { uz: "O'quvchi", ru: "Ученик", en: "Student", de: "Schüler" })}</th><th className="px-4 py-3 text-center">{tr(locale, { uz: "Miqdori", ru: "Кол-во", en: "Count", de: "Anzahl" })}</th>
                <th className="px-4 py-3">{tr(locale, { uz: "Guruh", ru: "Группа", en: "Group", de: "Gruppe" })}</th><th className="px-4 py-3">{tr(locale, { uz: "Kurs", ru: "Курс", en: "Course", de: "Kurs" })}</th><th className="px-4 py-3">{tr(locale, { uz: "Kim bekor qildi?", ru: "Кто отменил?", en: "Cancelled by?", de: "Storniert von?" })}</th><th className="px-4 py-3">{tr(locale, { uz: "Sana", ru: "Дата", en: "Date", de: "Datum" })}</th>
              </tr>
            </thead>
