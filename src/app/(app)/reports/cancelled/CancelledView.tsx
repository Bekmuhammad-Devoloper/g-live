...
export default function CancelledView({ rows, locale }: { rows: VCancel[]; locale: Locale }) {
  ...
  const doExport = () => {
    exportRows(
      ...
      [
        { key: "n", label: "№" },
        { key: "studentName", label: tr(locale, { uz: "Ism", ru: "Имя", en: "Name", de: "Name" }) },
        { key: "amount", label: tr(locale, { uz: "Bekor qilingan summa", ru: "Отменённая сумма", en: "Cancelled amount", de: "Stornierter Betrag" }) },
        { key: "totalCancelled", label: tr(locale, { uz: "Jami bekor qilingan", ru: "Всего отменено", en: "Total cancelled", de: "Gesamt storniert" }) },
        ...
        { key: "cancelledAt", label: tr(locale, { uz: "Sana", ru: "Дата", en: "Date", de: "Datum" }) },
      ],
      filtered.map((r, i) => ({
        n: i + 1,
        ...
        <div className="flex h-11 items-center gap-2 rounded-lg border border-slate-200 bg-white px-3 dark:border-slate-700 dark:bg-slate-900">
          ...
          {from || to ? (
            <>
              ...
              <button onClick={() => { setFrom(""); setTo(""); }} className="text-slate-400 hover:text-slate-600" title={tr(locale, { uz: "Tozalash", ru: "Очистить", en: "Clear", de: "Zurücksetzen" })}>✕</button>
            </>
          ) : (
            <button onClick={() => setFrom(new Date().toISOString().slice(0, 10))} className="text-sm text-slate-500 dark:text-slate-400">
              {tr(locale, { uz: "Oraliqni tanlang", ru: "Выберите диапазон", en: "Select range", de: "Zeitraum auswählen" })}
            </button>
          )}
        </div>
      ...
      <div className="overflow-hidden rounded-2xl border border-slate-200/70 bg-white shadow-card dark:border-slate-800 dark:bg-slate-900">
        <div className="flex items-center justify-end border-b border-slate-100 px-5 py-3 dark:border-slate-800">
          <span className="rounded-lg border border-slate-200 px-3 py-1.5 text-sm text-slate-500 dark:border-slate-700 dark:text-slate-300">
            {tr(locale, { uz: "Umumiy soni", ru: "Всего", en: "Total count", de: "Gesamtzahl" })}: <span className="font-bold text-slate-800 dark:text-slate-100">{filtered.length}</span>
          </span>
        </div>
          ...
          <table className="w-full text-left text-sm">
            <thead className="border-b border-slate-200/70 text-[12px] font-semibold uppercase tracking-wider text-slate-500 dark:border-slate-800 dark:text-slate-400">
              <tr>
                ...
                <th className="px-4 py-3.5">{tr(locale, { uz: "Ism", ru: "Имя", en: "Name", de: "Name" })}</th>
                <th className="px-4 py-3.5">{tr(locale, { uz: "Bekor qilingan summa", ru: "Отменённая сумма", en: "Cancelled amount", de: "Stornierter Betrag" })}</th>
                <th className="px-4 py-3.5">{tr(locale, { uz: "Jami bekor qilingan", ru: "Всего отменено", en: "Total cancelled", de: "Gesamt storniert" })}</th>
                ...
                <th className="px-4 py-3.5">{tr(locale, { uz: "Izoh", ru: "Комментарий", en: "Comment", de: "Kommentar" })}</th>
              </tr>
            </thead>
              ...
              {filtered.length === 0 ? (
                <tr>
                  <td colSpan={7} className="py-20 text-center">
                    ...
                    <div className="mt-3 text-base font-medium text-slate-500 dark:text-slate-400">{tr(locale, { uz: "Ma'lumotlar topilmadi", ru: "Данные не найдены", en: "No data found", de: "Keine Daten gefunden" })}</div>
                    <div className="mt-1 text-sm text-slate-400">{tr(locale, { uz: "Yozuvlar yo'q. Filtr yoki qidiruvni o'zgartirib ko'ring.", ru: "Записей нет. Попробуйте изменить фильтр или поиск.", en: "No records. Try changing the filter or search.", de: "Keine Einträge. Ändern Sie den Filter oder die Suche." })}</div>
                  </td>
                </tr>
              ) : (
                filtered.map((r, i) => (
