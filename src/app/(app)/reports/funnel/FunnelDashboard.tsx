...
const REPORTS: { n: number; label: TL; stages: string[] | null }[] = [
  { n: 1, label: { uz: "Barcha buyurtmalar soni", ru: "Всего заявок", en: "Total orders" }, stages: null },
  { n: 2, label: { uz: "Buyurtmadan ketganlar", ru: "Отказались от заявки", en: "Dropped the order" }, stages: ["LOST"] },
  { n: 3, label: { uz: "Sinov darsiga yozilganlar", ru: "Записались на пробный урок", en: "Signed up for trial lesson" }, stages: ["TEST", "OFFER", "AWAITING_PAYMENT", "PAID", "WON"] },
  { n: 4, label: { uz: "Sinov darsiga kelmay ketganlar", ru: "Не пришли на пробный урок", en: "Did not attend trial lesson" }, stages: [] },
  { n: 5, label: { uz: "Sinov darsiga kelganlar", ru: "Пришли на пробный урок", en: "Attended trial lesson" }, stages: ["OFFER", "AWAITING_PAYMENT", "PAID", "WON"] },
  { n: 6, label: { uz: "Sinov darsiga kelib ketganlar", ru: "Пришли на пробный урок и ушли", en: "Attended trial lesson and left" }, stages: [] },
  { n: 7, label: { uz: "Birinchi to'lovni qilganlar", ru: "Внесли первый платёж", en: "Made first payment" }, stages: ["PAID", "WON"] },
  { n: 8, label: { uz: "Birinchi to'lovni qilib ketganlar", ru: "Внесли первый платёж и ушли", en: "Made first payment and left" }, stages: [] },
  { n: 9, label: { uz: "Tugatganlar", ru: "Завершили", en: "Completed" }, stages: ["WON"] },
  { n: 10, label: { uz: "Boshqa filialdan ko'chirilgan", ru: "Переведены из другого филиала", en: "Transferred from another branch" }, stages: [] },
  { n: 11, label: { uz: "Boshqa filialga ko'chirilgan", ru: "Переведены в другой филиал", en: "Transferred to another branch" }, stages: [] },
];
...
export default function FunnelDashboard({ locale, leads, courses, sources, marketings, managers, defaultFrom, defaultTo }: Props) {
  ...
  const groupBy = (key: (l: FLead) => string | null) => {
    ...
    for (const l of filtered) {
      const k = key(l) || tr(locale, { uz: "Noma'lum", ru: "Неизвестно", en: "Unknown" });
      m.set(k, (m.get(k) ?? 0) + 1);
    }
  ...
  const handleExport = () => {
    exportRows(
      ...
      [
        { key: "n", label: "№" },
        { key: "label", label: tr(locale, { uz: "Hisobot turlari", ru: "Виды отчётов", en: "Report types" }) },
        { key: "count", label: tr(locale, { uz: "Soni", ru: "Количество", en: "Count" }) },
        { key: "courses", label: tr(locale, { uz: "Kurslar soni", ru: "Кол-во курсов", en: "Courses count" }) },
      ],
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap items-center gap-2.5">
        ...
        {marketings.length > 0
          ? <Sel value={marketing} onChange={setMarketing} placeholder={tr(locale, { uz: "Marketing", ru: "Маркетинг", en: "Marketing" })}>{marketings.map((m) => <option key={m} value={m}>{m}</option>)}</Sel>
          : <SelDisabled label={tr(locale, { uz: "Marketing", ru: "Маркетинг", en: "Marketing" })} soon={tr(locale, { uz: "Tez orada", ru: "Скоро", en: "Coming soon" })} />}
        <Sel value={course} onChange={setCourse} placeholder={tr(locale, { uz: "Kurs", ru: "Курс", en: "Course" })}>{courses.map((c) => <option key={c} value={c}>{c}</option>)}</Sel>
        <SelDisabled label={tr(locale, { uz: "Subkurs", ru: "Подкурс", en: "Subcourse" })} soon={tr(locale, { uz: "Tez orada", ru: "Скоро", en: "Coming soon" })} />
        {managers.length > 0
          ? <Sel value={manager} onChange={setManager} placeholder={tr(locale, { uz: "Moderator", ru: "Модератор", en: "Moderator" })}>{managers.map((m) => <option key={m.id} value={m.id}>{m.name}</option>)}</Sel>
          : <SelDisabled label={tr(locale, { uz: "Moderator", ru: "Модератор", en: "Moderator" })} soon={tr(locale, { uz: "Tez orada", ru: "Скоро", en: "Coming soon" })} />}
        <SelDisabled label={tr(locale, { uz: "O'qituvchi", ru: "Преподаватель", en: "Teacher" })} soon={tr(locale, { uz: "Tez orada", ru: "Скоро", en: "Coming soon" })} />
        <Sel value={source} onChange={setSource} placeholder={tr(locale, { uz: "Manba", ru: "Источник", en: "Source" })}>{sources.map((c) => <option key={c} value={c}>{c}</option>)}</Sel>
        <button
      ...
      <div className="grid gap-4 lg:grid-cols-2">
              ...
              <table className="w-full text-left text-sm">
                <thead className="border-b border-slate-200/70 text-[13px] font-semibold text-slate-600 dark:border-slate-800 dark:text-slate-300">
                  <tr>
                    ...
                    <th className="px-3 py-3">{tr(locale, { uz: "Hisobot turlari", ru: "Виды отчётов", en: "Report types" })}</th>
                    <th className="px-3 py-3 text-center">{tr(locale, { uz: "Soni", ru: "Количество", en: "Count" })}</th>
                    <th className="px-3 py-3 text-center">{tr(locale, { uz: "Kurslar soni", ru: "Кол-во курсов", en: "Courses count" })}</th>
                  </tr>
                </thead>
        ...
        <Card>
          <div className="mb-3 flex gap-1.5">
            {([["student", tr(locale, { uz: "O'quvchi", ru: "Ученик", en: "Student" }), "user"], ["course", tr(locale, { uz: "Kurs", ru: "Курс", en: "Course" }), "book"]] as const).map(([k, lb, ic]) => (
          ...
          {(sideTab === "course" ? courseDist : reportRows.filter((r) => r.count > 0).map((r) => ({ label: r.label, value: r.count }))).length === 0 ? (
            <div className="flex h-40 items-center justify-center rounded-xl border border-dashed border-slate-200 text-sm text-slate-400 dark:border-slate-700">{tr(locale, { uz: "Ma'lumot mavjud emas", ru: "Данные отсутствуют", en: "No data available" })}</div>
          ) : (
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        <Card>
          <h3 className="mb-4 text-center text-base font-bold text-slate-800 dark:text-slate-100">{tr(locale, { uz: "Lidlar tahlili", ru: "Анализ лидов", en: "Lead analysis" })} ({period === "day" ? tr(locale, { uz: "Kun", ru: "День", en: "Day" }) : period === "week" ? tr(locale, { uz: "Hafta", ru: "Неделя", en: "Week" }) : tr(locale, { uz: "Oy", ru: "Месяц", en: "Month" })})</h3>
          <TrendChart
            labels={trend.map((t) => t.label)}
            yLabel={tr(locale, { uz: "Lidlar soni", ru: "Кол-во лидов", en: "Number of leads" })}
            series={[
              { name: tr(locale, { uz: "Umumiy lidlar", ru: "Всего лидов", en: "Total leads" }), color: "#4148ef", values: trend.map((t) => t.total) },
              { name: tr(locale, { uz: "Yo'qotilgan lidlar", ru: "Потерянные лиды", en: "Lost leads" }), color: "#16a34a", values: trend.map((t) => t.lost) },
              { name: tr(locale, { uz: "Sotuvlar soni", ru: "Кол-во продаж", en: "Number of sales" }), color: "#f59e0b", values: trend.map((t) => t.sales) },
            ]}
          />
        </Card>
        <Card>
          <h3 className="mb-4 text-center text-base font-bold text-slate-800 dark:text-slate-100">{tr(locale, { uz: "Kurslar kesimida buyurtmalar taqsimoti", ru: "Распределение заявок по курсам", en: "Order distribution by course" })}</h3>
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        <Card>
          <div className="mb-4 flex flex-wrap gap-1.5 rounded-xl bg-slate-100 p-1 dark:bg-slate-800">
            {([["all", tr(locale, { uz: "Hammasi", ru: "Все", en: "All" })], ["progress", tr(locale, { uz: "Hozir ishlanayotgan lidlar", ru: "Лиды в работе", en: "Leads in progress" })], ["closed", tr(locale, { uz: "Yopilganlar", ru: "Закрытые", en: "Closed" })]] as const).map(([k, lb]) => (
          ...
          <div className="text-4xl font-bold text-slate-900 dark:text-slate-100">{leadTabCount} {tr(locale, { uz: "ta", ru: "шт.", en: "" })}</div>
          <div className="mt-1 text-sm text-slate-400">{tr(locale, { uz: "Lidlar soni", ru: "Кол-во лидов", en: "Number of leads" })}</div>
        </Card>
        <Card>
          <h3 className="mb-4 text-center text-base font-bold text-slate-800 dark:text-slate-100">{tr(locale, { uz: "So'rovnomalar kesimida buyurtmalar taqsimoti", ru: "Распределение заявок по источникам", en: "Order distribution by source" })}</h3>
