...
export default function LeaveFilters({ from, to, reason, reasons, locale }: { from: string; to: string; reason: string; reasons: string[]; locale: Locale }) {
  const router = useRouter();
  const sp = useSearchParams();
  const nav = (patch: Record<string, string>) => {
    const p = new URLSearchParams(sp.toString());
    for (const [k, v] of Object.entries(patch)) { if (v) p.set(k, v); else p.delete(k); }
    router.push(`/reports/leave-reasons?${p.toString()}`);
  };
  return (
