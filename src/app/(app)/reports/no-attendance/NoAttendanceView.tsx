    ...
    <div className="space-y-4">
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-2">
          <div className="text-sm font-semibold text-slate-700 dark:text-slate-200">{tr(locale, { uz: "Jami", ru: "Итого", en: "Total" })}: {filtered.length} {tr(locale, { uz: "ta dars", ru: "уроков", en: "lessons" })}</div>
          <button
        ...
        <div className="overflow-x-auto">
          <table className="w-full min-w-[720px] text-left text-sm">
            <thead className="border-b border-slate-200/70 bg-slate-50/80 text-[11px] font-semibold uppercase tracking-wider text-slate-500 dark:border-slate-800 dark:bg-slate-800/50">
              <tr>
                <th className="w-12 px-4 py-3">№</th><th className="px-4 py-3">{tr(locale, { uz: "Nomi", ru: "Название", en: "Name" })}</th><th className="px-4 py-3">{tr(locale, { uz: "Sana", ru: "Дата", en: "Date" })}</th><th className="px-4 py-3">{tr(locale, { uz: "O'qituvchi", ru: "Преподаватель", en: "Teacher" })}</th>{canMark && <th className="px-4 py-3 text-right">{tr(locale, { uz: "Amal", ru: "Действие", en: "Action" })}</th>}
              </tr>
            </thead>
                  ...
                  {canMark && (
                    <td className="px-4 py-3 text-right">
                      <Link href={`/groups/${r.groupId}/lessons/${r.id}`} className="inline-flex items-center gap-1.5 rounded-lg bg-brand-600 px-3 py-1.5 text-xs font-semibold text-white transition hover:bg-brand-700">
                        <Icon name="check" className="h-3.5 w-3.5" /> {tr(locale, { uz: "Davomat qilish", ru: "Отметить посещаемость", en: "Mark attendance" })}
                      </Link>
                    </td>
                  )}
                </tr>
