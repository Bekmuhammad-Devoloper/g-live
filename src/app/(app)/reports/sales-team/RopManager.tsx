  ...
  return (
    <>
      <button onClick={() => setOpen(true)} className="flex h-10 items-center gap-1.5 rounded-lg bg-brand-600 px-4 text-sm font-semibold text-white shadow-sm transition hover:bg-brand-700">
        <Icon name="plus" className="h-4 w-4" /> {tr(locale, { uz: "ROP qo'shish", ru: "Добавить РОП", en: "Add ROP", de: "ROP hinzufügen" })}
      </button>
      ...
      <div className="animate-slide-in-right absolute right-0 top-0 flex h-full w-[360px] max-w-[86%] flex-col border-l border-slate-200 bg-white shadow-pop dark:border-white/10 dark:bg-slate-900" onMouseDown={(e) => e.stopPropagation()}>
        <div className="flex items-center justify-between border-b border-slate-100 px-5 py-4 dark:border-slate-800">
          <div>
            ...
            <p className="text-xs text-slate-400">{tr(locale, { uz: "Operatorni savdo bo'limi rahbari (ROP) qilib belgilang", ru: "Назначьте оператора руководителем отдела продаж (РОП)", en: "Set an operator as head of sales (ROP)", de: "Legen Sie einen Operator als Vertriebsleiter (ROP) fest" })}</p>
          </div>
        ...
        <div className="border-b border-slate-100 px-5 py-3 dark:border-slate-800">
          <div className="relative">
            ...
            <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder={tr(locale, { uz: "Operator qidirish...", ru: "Поиск оператора...", en: "Search operator...", de: "Operator suchen..." })} className="h-10 w-full rounded-lg border border-slate-200 bg-white pl-9 pr-3 text-sm outline-none focus:border-brand-400 dark:border-slate-700 dark:bg-slate-800 dark:text-slate-100" />
          </div>
        ...
        <div className="flex-1 divide-y divide-slate-100 overflow-y-auto dark:divide-slate-800">
          {shown.length === 0 ? (
            <div className="py-12 text-center text-sm text-slate-400">{tr(locale, { uz: "Operator topilmadi. Avval", ru: "Оператор не найден. Сначала добавьте оператора в разделе", en: "No operator found. First add an operator in the", de: "Kein Operator gefunden. Fügen Sie zunächst einen Operator im Bereich" })} <b>{tr(locale, { uz: "Operatorlar", ru: "Операторы", en: "Operators", de: "Operatoren" })}</b> {tr(locale, { uz: "bo'limida operator qo'shing.", ru: ".", en: "section.", de: "hinzu." })}</div>
          ) : shown.map((o) => (
            <div key={o.id} className="flex items-center justify-between px-5 py-3">
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  ...
                  <div className="text-[11px]">{o.isRop ? <span className="font-bold text-amber-600 dark:text-amber-400">ROP</span> : <span className="text-slate-400">{tr(locale, { uz: "Operator", ru: "Оператор", en: "Operator", de: "Operator" })}</span>}</div>
                </div>
              </div>
              <button onClick={() => toggle(o)} disabled={pending && busy === o.id} className={cn("h-8 rounded-lg px-3 text-xs font-semibold transition disabled:opacity-60", o.isRop ? "border border-slate-200 text-slate-500 hover:bg-slate-50 dark:border-slate-700 dark:text-slate-300 dark:hover:bg-slate-800" : "bg-amber-500 text-white hover:bg-amber-600")}>
                {busy === o.id ? "..." : o.isRop ? tr(locale, { uz: "Bekor qilish", ru: "Отменить", en: "Cancel", de: "Abbrechen" }) : tr(locale, { uz: "ROP qilish", ru: "Сделать РОП", en: "Make ROP", de: "Zum ROP machen" })}
              </button>
            </div>
          ))}
        </div>

        <div className="border-t border-slate-100 px-5 py-3 text-xs text-slate-400 dark:border-slate-800">{tr(locale, { uz: `Hozir ${ropCount} ta ROP belgilangan`, ru: `Сейчас назначено РОП: ${ropCount}`, en: `Currently ${ropCount} ROP assigned`, de: `Derzeit ${ropCount} ROP zugewiesen` })}</div>
      </div>
    </div>,
    document.body,
  );
