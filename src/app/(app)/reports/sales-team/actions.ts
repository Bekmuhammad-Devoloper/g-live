...
export async function setRop(userId: string, isRop: boolean): Promise<{ ok?: boolean; error?: string }> {
  ...
  if (!CAN.includes(s.role as never)) return { error: tr(s.locale, { uz: "Ruxsat yo'q", ru: "Нет доступа", en: "No permission", de: "Keine Berechtigung" }) };
  const u = await prisma.user.findUnique({ where: { id: userId }, select: { id: true, role: true } });
  if (!u || u.role !== ROLES.OPERATOR) return { error: tr(s.locale, { uz: "Faqat operatorni ROP qilish mumkin", ru: "РОП можно назначить только оператора", en: "Only an operator can be made ROP", de: "Nur ein Operator kann zum ROP ernannt werden" }) };
