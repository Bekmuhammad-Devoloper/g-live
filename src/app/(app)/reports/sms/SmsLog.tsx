  ...
  const handleExport = () => {
    exportRows(
      ...
      [
        ...
        { key: "recipient", label: tr(locale, { uz: "Qabul qiluvchi", ru: "Получатель", en: "Recipient", de: "Empfänger" }) },
        { key: "channelLabel", label: tr(locale, { uz: "Kanal", ru: "Канал", en: "Channel", de: "Kanal" }) },
        { key: "title", label: tr(locale, { uz: "Sarlavha", ru: "Заголовок", en: "Title", de: "Titel" }) },
        { key: "body", label: tr(locale, { uz: "Matn", ru: "Текст", en: "Text", de: "Text" }) },
      ...
      filtered.map((r) => ({
        ...
        title: r.title,
        body: r.body,
        status: r.isRead ? tr(locale, { uz: "O'qilgan", ru: "Прочитано", en: "Read", de: "Gelesen" }) : tr(locale, { uz: "Yangi", ru: "Новое", en: "New", de: "Neu" }),
      }))
    );
  };
  ...
  return (
    <Card padded={false}>
      <div className="flex flex-wrap items-center gap-3 border-b border-slate-100 px-5 py-4 dark:border-slate-800">
        <h3 className="text-sm font-semibold text-slate-700">{tr(locale, { uz: "Oxirgi xabarlar", ru: "Последние сообщения", en: "Recent messages", de: "Letzte Nachrichten" })}</h3>
        ...
        <div className="relative">
          ...
          >
            <option value="">{tr(locale, { uz: "Barcha kanallar", ru: "Все каналы", en: "All channels", de: "Alle Kanäle" })}</option>
            {channels.map((c) => (
        ...
        >
          <Icon name="download" className="h-4 w-4" /> {tr(locale, { uz: "Eksport", ru: "Экспорт", en: "Export", de: "Exportieren" })}
        </button>
        <span className="ml-auto text-xs text-slate-400">{tr(locale, { uz: "O'qilgan", ru: "Прочитано", en: "Read", de: "Gelesen" })}: {readCount} / {filtered.length}</span>
      </div>
      <Table
        head={
          <tr>
            <th className="px-4 py-3">{tr(locale, { uz: "Sana", ru: "Дата", en: "Date", de: "Datum" })}</th>
            <th className="px-4 py-3">{tr(locale, { uz: "Qabul qiluvchi", ru: "Получатель", en: "Recipient", de: "Empfänger" })}</th>
            <th className="px-4 py-3">{tr(locale, { uz: "Kanal", ru: "Канал", en: "Channel", de: "Kanal" })}</th>
            <th className="px-4 py-3">{tr(locale, { uz: "Sarlavha", ru: "Заголовок", en: "Title", de: "Titel" })}</th>
            <th className="px-4 py-3">{tr(locale, { uz: "Matn", ru: "Текст", en: "Text", de: "Text" })}</th>
            <th className="px-4 py-3">{tr(locale, { uz: "Holat", ru: "Статус", en: "Status", de: "Status" })}</th>
          </tr>
        }
      >
        {filtered.length === 0 ? (
          <EmptyRow colSpan={6} text={tr(locale, { uz: "Xabar yo'q", ru: "Нет сообщений", en: "No messages", de: "Keine Nachrichten" })} />
        ) : (
          filtered.map((n) => (
            <tr key={n.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/50">
              ...
              <td className="px-4 py-3">
                {n.isRead ? <Badge tone="green">{tr(locale, { uz: "O'qilgan", ru: "Прочитано", en: "Read", de: "Gelesen" })}</Badge> : <Badge tone="amber">{tr(locale, { uz: "Yangi", ru: "Новое", en: "New", de: "Neu" })}</Badge>}
              </td>
            </tr>
          ))
        )}
