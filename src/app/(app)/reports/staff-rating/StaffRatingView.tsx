...
export default function StaffRatingView({ rows, opts, locale }: { rows: VRating[]; opts: Opts; locale: Locale }) {
  ...
  const onExport = () => {
    exportRows(
      ...
      [
        ...
        { key: "course", label: tr(locale, { uz: "Kurs", ru: "Курс", en: "Course", de: "Kurs" }) },
        { key: "group", label: tr(locale, { uz: "Guruh", ru: "Группа", en: "Group", de: "Gruppe" }) },
        { key: "lessonDate", label: tr(locale, { uz: "Dars sanasi", ru: "Дата урока", en: "Lesson date", de: "Unterrichtsdatum" }) },
        ...
        { key: "rating", label: tr(locale, { uz: "Baho", ru: "Оценка", en: "Rating", de: "Bewertung" }) },
      ],
      filtered.map((r) => ({
  ...
  const SortTh = ({ col, label, className }: { col: string; label: string; className?: string }) => (
    <th className={cn("px-4 py-3", className)}>
      <button type="button" onClick={() => toggleSort(col)} className="inline-flex items-center gap-1 hover:text-brand-600 dark:hover:text-brand-300" title={tr(locale, { uz: "Saralash", ru: "Сортировка", en: "Sort", de: "Sortieren" })}>
        <span>{label}</span>
    ...
    <div className="space-y-3">
      ...
      <div className="flex flex-wrap items-center gap-2">
        <div className="flex h-11 items-center gap-1 rounded-xl border border-slate-200 bg-white px-3 dark:border-slate-700 dark:bg-slate-800">
          ...
          <input type="date" value={from} onChange={(e) => { setFrom(e.target.value); p1(); }} className="w-[110px] bg-transparent text-sm text-slate-700 outline-none dark:text-slate-100" placeholder={tr(locale, { uz: "dan", ru: "с", en: "from", de: "von" })} />
        ...
        <Sel v={izoh} set={(x) => { setIzoh(x); p1(); }} ph={tr(locale, { uz: "Izoh", ru: "Комментарий", en: "Comment", de: "Kommentar" })} opts={[{ v: "bor", label: tr(locale, { uz: "Izoh bor", ru: "Есть комментарий", en: "Has comment", de: "Mit Kommentar" }) }, { v: "yoq", label: tr(locale, { uz: "Izoh yo'q", ru: "Нет комментария", en: "No comment", de: "Ohne Kommentar" }) }]} />
        <Sel v={group} set={(x) => { setGroup(x); p1(); }} ph={tr(locale, { uz: "Guruh", ru: "Группа", en: "Group", de: "Gruppe" })} opts={opts.groups.map((x) => ({ v: x, label: x }))} />
        <Sel v={course} set={(x) => { setCourse(x); p1(); }} ph={tr(locale, { uz: "Kurs", ru: "Курс", en: "Course", de: "Kurs" })} opts={opts.courses.map((x) => ({ v: x, label: x }))} />
        <Sel v={teacher} set={(x) => { setTeacher(x); p1(); }} ph={tr(locale, { uz: "O'qituvchi", ru: "Преподаватель", en: "Teacher", de: "Lehrer" })} opts={opts.teachers.map((x) => ({ v: x, label: x }))} />
        <Sel v={student} set={(x) => { setStudent(x); p1(); }} ph={tr(locale, { uz: "O'quvchi", ru: "Ученик", en: "Student", de: "Schüler" })} opts={opts.students.map((x) => ({ v: x, label: x }))} />
        <Sel v={rating} set={(x) => { setRating(x); p1(); }} ph={tr(locale, { uz: "Reyting", ru: "Рейтинг", en: "Rating", de: "Bewertung" })} opts={[5, 4, 3, 2, 1].map((n) => ({ v: String(n), label: `${n} ⭐` }))} />
      ...
      <div className="text-sm text-slate-500 dark:text-slate-400">{tr(locale, { uz: "O'rtacha reyting", ru: "Средний рейтинг", en: "Average rating", de: "Durchschnittliche Bewertung" })}: <b className="text-amber-600 dark:text-amber-400">{avg}</b></div>
          ...
          <table className="w-full min-w-[1080px] text-left text-sm">
            <thead className="border-b border-slate-200/70 bg-slate-50/80 text-[11px] font-semibold uppercase tracking-wider text-slate-500 dark:border-slate-800 dark:bg-slate-800/50">
              <tr>
                ...
                <SortTh col="teacher" label={tr(locale, { uz: "O'qituvchi", ru: "Преподаватель", en: "Teacher", de: "Lehrer" })} />
                <SortTh col="student" label={tr(locale, { uz: "O'quvchi", ru: "Ученик", en: "Student", de: "Schüler" })} />
                <SortTh col="course" label={tr(locale, { uz: "Kurs", ru: "Курс", en: "Course", de: "Kurs" })} />
                <SortTh col="group" label={tr(locale, { uz: "Guruh", ru: "Группа", en: "Group", de: "Gruppe" })} />
                <SortTh col="lessonDate" label={tr(locale, { uz: "Dars sanasi", ru: "Дата урока", en: "Lesson date", de: "Unterrichtsdatum" })} />
                <th className="px-4 py-3">{tr(locale, { uz: "Izoh", ru: "Комментарий", en: "Comment", de: "Kommentar" })}</th>
                <SortTh col="date" label={tr(locale, { uz: "Sana", ru: "Дата", en: "Date", de: "Datum" })} />
                <SortTh col="rating" label={tr(locale, { uz: "Baho", ru: "Оценка", en: "Rating", de: "Bewertung" })} className="text-center" />
              </tr>
            </thead>
