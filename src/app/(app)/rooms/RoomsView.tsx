  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        <h1 className="text-[26px] font-bold tracking-tight text-slate-900 dark:text-slate-100">{tr(locale, { uz: "Xonalar", ru: "Кабинеты", en: "Rooms", de: "Räume" })}</h1>
        {canManage && (
          <button onClick={() => setDrawer({ edit: null })} className="flex h-11 items-center gap-2 rounded-full bg-brand-800 px-6 text-sm font-semibold text-white shadow-sm transition hover:bg-brand-900">
            {tr(locale, { uz: "Yangisini qo'shish", ru: "Добавить новый", en: "Add new", de: "Neu hinzufügen" })}
          </button>
        )}
      </div>
      ...
      {rooms.length > 6 && (
        <div className="relative max-w-xs">
          ...
          <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder={tr(locale, { uz: "Xona qidirish...", ru: "Поиск кабинета...", en: "Search room...", de: "Raum suchen..." })} className="h-10 w-full rounded-lg border border-slate-200 bg-white pl-9 pr-3 text-sm outline-none focus:border-brand-400 dark:border-slate-700 dark:bg-slate-800 dark:text-slate-100" />
        </div>
      )}
          ...
          <table className="w-full min-w-[620px] text-left text-sm">
            <thead className="border-b border-slate-200/70 bg-slate-50/60 text-[11px] font-semibold uppercase tracking-wider text-slate-500 dark:border-slate-800 dark:bg-slate-800/40 dark:text-slate-400">
              <tr>
                ...
                <th className="px-6 py-3.5">{tr(locale, { uz: "Xona nomi", ru: "Название", en: "Room name", de: "Raumname" })}</th>
                <th className="px-6 py-3.5">{tr(locale, { uz: "Sig'imi", ru: "Вместимость", en: "Capacity", de: "Kapazität" })}</th>
                <th className="px-6 py-3.5">{tr(locale, { uz: "Izoh", ru: "Примечание", en: "Note", de: "Notiz" })}</th>
                <th className="px-6 py-3.5 text-right">{tr(locale, { uz: "Amallar", ru: "Действия", en: "Actions", de: "Aktionen" })}</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
              {shown.length === 0 ? (
                <tr><td colSpan={5} className="py-16 text-center text-slate-400">{tr(locale, { uz: "Xona topilmadi", ru: "Кабинет не найден", en: "No rooms found", de: "Keine Räume gefunden" })}</td></tr>
              ) : shown.map((r, i) => (
                <tr key={r.id} className="group transition hover:bg-slate-50 dark:hover:bg-slate-800/40">
                  ...
                  <td className="px-6 py-4">
                    <div className="flex items-center justify-end gap-2.5">
                      ...
                      <Link href={`/rooms/${r.id}`} title={tr(locale, { uz: "Xona ma'lumotlari", ru: "Информация о кабинете", en: "Room details", de: "Rauminformationen" })} className="grid h-8 w-8 place-items-center rounded-lg text-slate-400 transition hover:bg-slate-100 hover:text-brand-600 dark:hover:bg-slate-800"><Icon name="building" className="h-4 w-4" /></Link>
                      {canManage && (
                        <>
                          <button onClick={() => setDrawer({ edit: r })} title={tr(locale, { uz: "Tahrirlash", ru: "Редактировать", en: "Edit", de: "Bearbeiten" })} className="grid h-8 w-8 place-items-center rounded-lg text-slate-400 transition hover:bg-slate-100 hover:text-brand-600 dark:hover:bg-slate-800"><Icon name="pencil" className="h-4 w-4" /></button>
                          <button onClick={() => onDelete(r)} title={tr(locale, { uz: "O'chirish", ru: "Удалить", en: "Delete", de: "Löschen" })} className="grid h-8 w-8 place-items-center rounded-lg text-rose-500 transition hover:bg-rose-50 dark:hover:bg-rose-500/10"><Icon name="trash" className="h-4 w-4" /></button>
                        </>
                      )}
                    </div>
                  </td>
...
function durStr(m: number, locale: Locale): string {
  ...
  const soat = tr(locale, { uz: "soat", ru: "ч", en: "h", de: "Std." });
  const daq = tr(locale, { uz: "daq", ru: "мин", en: "m", de: "Min." });
...
function StatusPill({ status, locale }: { status: RoomStatus; locale: Locale }) {
  if (status.busy) {
    return (
      <span
        title={`${status.course ?? ""}${status.until ? ` · ${tr(locale, { uz: `${status.until} gacha`, ru: `до ${status.until}`, en: `until ${status.until}`, de: `bis ${status.until}` })}` : ""}`}
  ...
  return (
    <span className="inline-flex items-center gap-1.5 whitespace-nowrap rounded-full bg-slate-100 px-2.5 py-1 text-xs font-medium text-slate-500 dark:bg-slate-800 dark:text-slate-400">
      ...
      {tr(locale, { uz: "Bo'sh", ru: "Свободно", en: "Free", de: "Frei" })}
      {status.until ? (
        <span className="opacity-80">· {tr(locale, { uz: `${status.until} gacha`, ru: `до ${status.until}`, en: `until ${status.until}`, de: `bis ${status.until}` })}{status.freeMin != null ? ` · ${durStr(status.freeMin, locale)}` : ""}</span>
      ) : (
        <span className="opacity-70">· {tr(locale, { uz: "kun oxirigacha bo'sh", ru: "свободно до конца дня", en: "free until end of day", de: "frei bis Tagesende" })}</span>
      )}
    </span>
  );
}
...
function RoomDrawer({ locale, edit, onClose, onSaved }: { locale: Locale; edit: VRoom | null; onClose: () => void; onSaved: (msg: string) => void }) {
  ...
  const submit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    start(async () => {
      ...
      if (r.ok) onSaved(edit ? tr(locale, { uz: "Saqlandi", ru: "Сохранено", en: "Saved", de: "Gespeichert" }) : tr(locale, { uz: "Xona qo'shildi", ru: "Кабинет добавлен", en: "Room added", de: "Raum hinzugefügt" }));
      ...
      <form onSubmit={submit} onMouseDown={(e) => e.stopPropagation()} className="animate-slide-in-right absolute right-0 top-0 flex h-full w-[380px] max-w-[88%] flex-col border-l border-slate-200 bg-white shadow-pop dark:border-white/10 dark:bg-[#15243d]">
        ...
        <div className="flex-1 space-y-4 overflow-y-auto px-5 py-5">
          <label className="block">
            <span className="mb-1.5 block text-sm font-medium text-slate-600 dark:text-slate-300">{tr(locale, { uz: "Ism", ru: "Название", en: "Name", de: "Name" })}</span>
            <input name="name" required defaultValue={edit?.name ?? ""} placeholder={tr(locale, { uz: "Masalan: Room #1", ru: "Например: Room #1", en: "e.g. Room #1", de: "z. B. Room #1" })} className={inp} autoFocus />
          </label>
          <label className="block">
            <span className="mb-1.5 block text-sm font-medium text-slate-600 dark:text-slate-300">{tr(locale, { uz: "Xona sig'imi", ru: "Вместимость", en: "Capacity", de: "Kapazität" })}</span>
            <input name="capacity" type="number" min={0} max={1000} defaultValue={edit?.capacity ?? ""} placeholder="0" className={inp} />
          </label>
          <label className="block">
            <span className="mb-1.5 block text-sm font-medium text-slate-600 dark:text-slate-300">{tr(locale, { uz: "Izoh", ru: "Примечание", en: "Note", de: "Notiz" })} <span className="text-slate-400">({tr(locale, { uz: "ixtiyoriy", ru: "необязательно", en: "optional", de: "optional" })})</span></span>
            <input name="note" defaultValue={edit?.note ?? ""} placeholder={tr(locale, { uz: "Qo'shimcha ma'lumot", ru: "Дополнительная информация", en: "Additional info", de: "Zusätzliche Informationen" })} className={inp} />
          </label>
        ...
        <div className="shrink-0 border-t border-slate-100 px-5 py-4 dark:border-white/10">
          <button type="submit" disabled={pending} className="h-11 rounded-full bg-brand-800 px-8 text-sm font-semibold text-white transition hover:bg-brand-900 disabled:opacity-60">{pending ? tr(locale, { uz: "Saqlanmoqda...", ru: "Сохранение...", en: "Saving...", de: "Wird gespeichert..." }) : tr(locale, { uz: "Saqlash", ru: "Сохранить", en: "Save", de: "Speichern" })}</button>
        </div>
      </form>
    </div>,
    document.body,
