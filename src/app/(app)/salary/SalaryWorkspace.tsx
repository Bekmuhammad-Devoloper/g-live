...
const MONTHS_ML: { uz: string; ru: string; en: string; de: string }[] = [
  { uz: "Yanvar", ru: "Январь", en: "January", de: "Januar" },
  { uz: "Fevral", ru: "Февраль", en: "February", de: "Februar" },
  { uz: "Mart", ru: "Март", en: "March", de: "März" },
  { uz: "Aprel", ru: "Апрель", en: "April", de: "April" },
  { uz: "May", ru: "Май", en: "May", de: "Mai" },
  { uz: "Iyun", ru: "Июнь", en: "June", de: "Juni" },
  { uz: "Iyul", ru: "Июль", en: "July", de: "Juli" },
  { uz: "Avgust", ru: "Август", en: "August", de: "August" },
  { uz: "Sentabr", ru: "Сентябрь", en: "September", de: "September" },
  { uz: "Oktabr", ru: "Октябрь", en: "October", de: "Oktober" },
  { uz: "Noyabr", ru: "Ноябрь", en: "November", de: "November" },
  { uz: "Dekabr", ru: "Декабрь", en: "December", de: "Dezember" },
];
  ...
  const doExport = () => {
    exportRows(
      ...
      [
        ...
        { key: "fiksa", label: tr(locale, { uz: "Fiksa (so'm)", ru: "Фикс (сум)", en: "Base (UZS)", de: "Fixgehalt (UZS)" }) },
        { key: "lessons", label: tr(locale, { uz: "Darslar", ru: "Занятия", en: "Lessons", de: "Unterricht" }) },
        { key: "students", label: tr(locale, { uz: "O'quvchilar", ru: "Ученики", en: "Students", de: "Schüler" }) },
        { key: "bonus", label: tr(locale, { uz: "Bonus (so'm)", ru: "Бонус (сум)", en: "Bonus (UZS)", de: "Bonus (UZS)" }) },
        { key: "penalty", label: tr(locale, { uz: "Ushlanma (so'm)", ru: "Удержание (сум)", en: "Deduction (UZS)", de: "Abzug (UZS)" }) },
        { key: "kpi", label: tr(locale, { uz: "KPI (so'm)", ru: "KPI (сум)", en: "KPI (UZS)", de: "KPI (UZS)" }) },
        { key: "net", label: tr(locale, { uz: "Jami (so'm)", ru: "Итого (сум)", en: "Total (UZS)", de: "Gesamt (UZS)" }) },
      ...
      filtered.map((r) => ({
        ...
        status: !r.hasRecord ? tr(locale, { uz: "Hisoblanmagan", ru: "Не рассчитано", en: "Not calculated", de: "Nicht berechnet" }) : r.closed ? tr(locale, { uz: "Yopilgan", ru: "Закрыто", en: "Closed", de: "Geschlossen" }) : tr(locale, { uz: "Hisoblangan", ru: "Рассчитано", en: "Calculated", de: "Berechnet" }),
      })),
    );
  };
    ...
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap items-center gap-2 print:hidden">
        <div className="flex items-center gap-1.5 rounded-lg border border-slate-200 bg-white p-1 dark:border-slate-700 dark:bg-slate-800">
          <button onClick={prev} className="flex h-8 w-8 items-center justify-center rounded-md text-slate-500 transition hover:bg-slate-100 dark:hover:bg-slate-700" aria-label={tr(locale, { uz: "Oldingi oy", ru: "Предыдущий месяц", en: "Previous month", de: "Vorheriger Monat" })}>‹</button>
          ...
          <button onClick={next} className="flex h-8 w-8 items-center justify-center rounded-md text-slate-500 transition hover:bg-slate-100 dark:hover:bg-slate-700" aria-label={tr(locale, { uz: "Keyingi oy", ru: "Следующий месяц", en: "Next month", de: "Nächster Monat" })}>›</button>
        </div>
        ...
        <div className="relative">
          <input
            ...
            placeholder={tr(locale, { uz: "O'qituvchi qidirish", ru: "Поиск преподавателя", en: "Search teacher", de: "Lehrer suchen" })}
        ...
        <div className="ml-auto flex items-center gap-2">
          ...
          <button onClick={printPage} className="flex h-10 items-center gap-1.5 rounded-lg border border-slate-200 bg-white px-3 text-sm font-semibold text-slate-600 transition hover:bg-slate-50 dark:border-slate-700 dark:bg-slate-800 dark:text-slate-200 dark:hover:bg-slate-700">
            🖨 {tr(locale, { uz: "Chop etish", ru: "Печать", en: "Print", de: "Drucken" })}
          </button>
        </div>
      </div>
      ...
      <div className="overflow-hidden rounded-2xl border border-slate-200/70 bg-white shadow-card dark:border-slate-800 dark:bg-slate-900">
        <div className="flex items-center justify-between border-b border-slate-100 px-4 py-2 dark:border-slate-800">
          <span className="text-sm font-semibold text-slate-600 dark:text-slate-300">{monthName(locale, month)} {year} — {tr(locale, { uz: "hisob-kitob", ru: "расчёт", en: "calculation", de: "Berechnung" })}</span>
          <span className="rounded-lg bg-slate-100 px-2.5 py-1 text-xs font-semibold text-slate-500 dark:bg-slate-800 dark:text-slate-300">{tr(locale, { uz: "Fond", ru: "Фонд", en: "Fund", de: "Fonds" })}: {formatMoney(totalNet, locale)}</span>
        </div>
          ...
          <table className="w-full text-left text-sm" style={{ minWidth: 980 }}>
            <thead className="border-b border-slate-200/70 bg-slate-50/80 text-[11px] font-semibold uppercase tracking-wider text-slate-500 dark:border-slate-800 dark:bg-slate-800/50">
              <tr>
                ...
                <th className="px-4 py-3">{tr(locale, { uz: "Fiksa", ru: "Фикс", en: "Base", de: "Fixgehalt" })}</th>
                <th className="px-4 py-3">{tr(locale, { uz: "Darslar", ru: "Занятия", en: "Lessons", de: "Unterricht" })}</th>
                <th className="px-4 py-3">{tr(locale, { uz: "O'quvchilar", ru: "Ученики", en: "Students", de: "Schüler" })}</th>
                <th className="px-4 py-3">{tr(locale, { uz: "Bonus", ru: "Бонус", en: "Bonus", de: "Bonus" })}</th>
                <th className="px-4 py-3">{tr(locale, { uz: "Ushlanma", ru: "Удержание", en: "Deduction", de: "Abzug" })}</th>
                <th className="px-4 py-3">KPI</th>
                <th className="px-4 py-3">{tr(locale, { uz: "Jami", ru: "Итого", en: "Total", de: "Gesamt" })}</th>
                <th className="px-4 py-3">{tr(locale, { uz: "Holati", ru: "Статус", en: "Status", de: "Status" })}</th>
                {canManage && <th className="px-4 py-3 print:hidden">{tr(locale, { uz: "Amal", ru: "Действие", en: "Action", de: "Aktion" })}</th>}
              </tr>
            </thead>
              ...
              {filtered.length === 0 ? (
                ...
                <tr key={r.teacherId} className="hover:bg-slate-50 dark:hover:bg-slate-800/50">
                  ...
                  <td className="px-4 py-3">
                    {!r.hasRecord ? (
                      <span className="rounded-md bg-slate-400/20 px-2 py-0.5 text-[11px] font-semibold text-slate-500">{tr(locale, { uz: "Hisoblanmagan", ru: "Не рассчитано", en: "Not calculated", de: "Nicht berechnet" })}</span>
                    ) : r.closed ? (
                      <span className="rounded-md bg-slate-400/20 px-2 py-0.5 text-[11px] font-semibold text-slate-500">{tr(locale, { uz: "Yopilgan", ru: "Закрыто", en: "Closed", de: "Geschlossen" })}</span>
                    ) : (
                      <span className="rounded-md bg-emerald-500/15 px-2 py-0.5 text-[11px] font-semibold text-emerald-600 dark:text-emerald-400">{tr(locale, { uz: "Hisoblangan", ru: "Рассчитано", en: "Calculated", de: "Berechnet" })}</span>
                    )}
                  </td>
                  {canManage && (
                    <td className="px-4 py-3 print:hidden">
                      ...
                      >
                        {r.hasRecord ? tr(locale, { uz: "Tahrirlash", ru: "Редактировать", en: "Edit", de: "Bearbeiten" }) : tr(locale, { uz: "Hisoblash", ru: "Рассчитать", en: "Calculate", de: "Berechnen" })}
                      </button>
                    </td>
                  )}
                </tr>
    ...
    <div className="fixed inset-0 z-[80]" onMouseDown={onClose}>
      ...
      <form ref={formRef} action={action} onMouseDown={(e) => e.stopPropagation()} className="animate-slide-in-right absolute right-0 top-0 flex h-full w-[440px] max-w-[92%] flex-col border-l border-slate-200 bg-white shadow-pop dark:border-white/10 dark:bg-[#15243d]">
        <div className="flex shrink-0 items-center justify-between border-b border-slate-100 px-5 py-4 dark:border-white/10">
          <div>
            <h3 className="text-base font-bold text-slate-800 dark:text-slate-100">{tr(locale, { uz: "Maosh hisobi", ru: "Расчёт зарплаты", en: "Salary calculation", de: "Gehaltsberechnung" })}</h3>
        ...
        <div className="flex-1 space-y-3.5 overflow-y-auto px-5 py-4">
          ...
          <div className="grid grid-cols-2 gap-3 text-sm">
            <div className="rounded-lg bg-slate-50 px-3 py-2 dark:bg-slate-800/50">
              <div className="text-xs text-slate-400">{tr(locale, { uz: "Fiksa", ru: "Фикс", en: "Base", de: "Fixgehalt" })}</div>
              <div className="font-semibold tabular-nums text-slate-700 dark:text-slate-200">{formatMoney(row.fiksa, locale)}</div>
            </div>
            <div className="rounded-lg bg-slate-50 px-3 py-2 dark:bg-slate-800/50">
              <div className="text-xs text-slate-400">{tr(locale, { uz: "Darslar / O'quvchilar", ru: "Занятия / Ученики", en: "Lessons / Students", de: "Unterricht / Schüler" })}</div>
              <div className="font-semibold tabular-nums text-slate-700 dark:text-slate-200">{row.lessons} / {row.students}</div>
            </div>
          </div>

          <label className="block">
            <span className="mb-1 block text-xs font-medium text-slate-500 dark:text-slate-400">{tr(locale, { uz: "Bonus (so'm)", ru: "Бонус (сум)", en: "Bonus (UZS)", de: "Bonus (UZS)" })}</span>
            <input name="bonus" type="number" min="0" value={bonus} onChange={(e) => setBonus(Number(e.target.value))} className={inp} />
          </label>
          <label className="block">
            <span className="mb-1 block text-xs font-medium text-slate-500 dark:text-slate-400">{tr(locale, { uz: "Ushlanma (so'm)", ru: "Удержание (сум)", en: "Deduction (UZS)", de: "Abzug (UZS)" })}</span>
            <input name="penalty" type="number" min="0" value={penalty} onChange={(e) => setPenalty(Number(e.target.value))} className={inp} />
          </label>
          <label className="block">
            <span className="mb-1 block text-xs font-medium text-slate-500 dark:text-slate-400">{tr(locale, { uz: "KPI bonus (so'm)", ru: "KPI бонус (сум)", en: "KPI bonus (UZS)", de: "KPI-Bonus (UZS)" })}</span>
          ...
          <label className="flex items-center gap-2 pt-1">
            ...
            <span className="text-sm text-slate-600 dark:text-slate-300">{tr(locale, { uz: "Oyni yopish (tasdiqlash)", ru: "Закрыть месяц (утвердить)", en: "Close month (confirm)", de: "Monat schließen (bestätigen)" })}</span>
          </label>

          <div className="rounded-lg border border-brand-200 bg-brand-50 px-3 py-2.5 dark:border-brand-500/20 dark:bg-brand-500/10">
            <div className="text-xs text-slate-500 dark:text-slate-400">{tr(locale, { uz: "Jami to'lov", ru: "Итого к выплате", en: "Total payment", de: "Gesamtzahlung" })}</div>
            <div className="text-lg font-bold tabular-nums text-brand-700 dark:text-brand-300">{formatMoney(net, locale)}</div>
          </div>

          {state.error && <p className="rounded-lg bg-rose-50 px-3 py-2 text-sm text-rose-700 dark:bg-rose-500/10 dark:text-rose-400">{state.error === "forbidden" ? tr(locale, { uz: "Ruxsat yo'q.", ru: "Нет доступа.", en: "No access.", de: "Kein Zugriff." }) : tr(locale, { uz: "Ma'lumotlar to'liq emas.", ru: "Данные заполнены не полностью.", en: "Data is incomplete.", de: "Daten sind unvollständig." })}</p>}
        </div>
