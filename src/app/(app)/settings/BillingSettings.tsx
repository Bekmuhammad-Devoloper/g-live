...
export default function BillingSettings({ locale }: { locale: Locale }) {
  ...
  const monthWord = tr(locale, { uz: "oy", ru: "мес.", en: "mo" });
  const sumWord = tr(locale, { uz: "so'm", ru: "сум", en: "so'm" });
    ...
    <div className="space-y-6">
      <div className="grid gap-6 lg:grid-cols-2">
        ...
        <div className="rounded-2xl border border-slate-300 bg-white p-6 shadow-card dark:border-slate-600 dark:bg-slate-900">
          <h2 className="mb-5 text-xl font-bold text-slate-800 dark:text-slate-100">{tr(locale, { uz: "Platforma uchun to'lov", ru: "Оплата платформы", en: "Platform payment" })}</h2>
          <label className="mb-1.5 block text-sm font-medium text-slate-600 dark:text-slate-300">{tr(locale, { uz: "Davr", ru: "Период", en: "Period" })} <span className="text-rose-500">*</span></label>
          ...
          {msg && <p className="mt-3 text-sm text-emerald-600 dark:text-emerald-400">{tr(locale, { uz: "✓ To'lov qabul qilindi (demo) — tarixga qo'shildi.", ru: "✓ Платёж принят (демо) — добавлен в историю.", en: "✓ Payment accepted (demo) — added to history." })}</p>}
        </div>
                  ...
                  {PERIODS.map((pr) => {
                    ...
                    return (
                      <div key={pr.months} className="flex items-stretch gap-1">
                        <div className="flex w-8 shrink-0 items-center justify-center rounded bg-[#0f172a] text-[9px] font-bold leading-tight text-white">{pr.months}<br />{tr(locale, { uz: "OY", ru: "МЕС", en: "MO" })}</div>
                        <div className="relative flex-1 rounded bg-amber-400 px-1 py-1 text-center leading-tight">
                          ...
                          {pr.bonus > 0 && (
                            <span className="absolute -bottom-1 -right-1 rounded-full border border-[#0f172a] bg-[#ffffff] px-1 text-[7px] font-bold text-[#0f172a]">+{pr.bonus} {tr(locale, { uz: "OY", ru: "МЕС", en: "MO" })}</span>
                          )}
                        </div>
                      </div>
                    );
          ...
          <table className="w-full min-w-[560px] text-left text-sm">
            <thead className="border-b border-slate-300 text-[13px] font-semibold text-slate-600 dark:border-slate-600 dark:text-slate-300">
              <tr>
                ...
                <th className="px-5 py-3.5">{tr(locale, { uz: "Tarif", ru: "Тариф", en: "Plan" })}</th>
                <th className="px-5 py-3.5">{tr(locale, { uz: "Sana", ru: "Дата", en: "Date" })}</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
              {history.length === 0 ? (
                <tr><td colSpan={3} className="px-5 py-10 text-center text-slate-400">{tr(locale, { uz: "To'lov tarixi bo'sh", ru: "История платежей пуста", en: "Payment history is empty" })}</td></tr>
              ) : (
                history.map((h, i) => (
