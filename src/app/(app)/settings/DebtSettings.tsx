...
export default function DebtSettings({ locale, defaultFee }: { locale: Locale; defaultFee: number }) {
  ...
  const save = () => start(async () => {
    ...
    if (r.ok) { setSaved(true); setTimeout(() => setSaved(false), 2200); }
      ...
      ? L("Saqlanmadi — bu sozlama faqat direktor va o'rinbosariga.", "Не сохранено — только директор и заместитель.", "Not saved — director and deputy only.", "Nicht gespeichert — nur für Direktor und Stellvertreter.")
  ...
  return (
    <div className="max-w-xl">
      <h2 className="mb-1 text-2xl font-bold text-slate-800 dark:text-slate-100">
        {L("Qarzdorlik", "Задолженность", "Debt", "Schulden")}
      </h2>
      <p className="mb-6 text-sm text-slate-500 dark:text-slate-400">
        {L(
          ...
          "Die Schulden werden automatisch ab dem Monat berechnet, in dem der Schüler ins System aufgenommen wurde.",
        )}
      </p>
      ...
      <div className="rounded-2xl border border-slate-200/70 bg-white p-5 dark:border-slate-800 dark:bg-slate-900">
        <label className="mb-1 block text-xs font-semibold text-slate-600 dark:text-slate-300">
          {L("Umumiy oylik to'lov (so'm)", "Общая ежемесячная оплата (сум)", "Default monthly fee (UZS)", "Standardmäßige Monatsgebühr (UZS)")}
        </label>
        ...
        <div className="mt-3 flex items-start gap-1.5 text-xs leading-relaxed text-slate-500 dark:text-slate-400">
          ...
          <span>
            {L(
              ...
              "Wird für Monate verwendet, in denen der Schüler in keiner Gruppe ist. Andernfalls gilt der Gruppenpreis, sonst der Kurspreis. Bei 0 werden gruppenlose Monate nicht berechnet.",
            )}
          </span>
        </div>
        ...
        {value && Number(value) > 0 && (
          <p className="mt-2 text-xs text-slate-400">
            {L("Yiliga", "В год", "Per year", "Pro Jahr")}: <b className="text-slate-600 dark:text-slate-300">{formatMoney(Number(value) * 12, locale)}</b>
          </p>
        )}
        ...
        <div className="mt-4 flex items-center gap-3">
          ...
          {saved && (
            <span className="flex items-center gap-1 text-sm font-medium text-emerald-600 dark:text-emerald-400">
              <Icon name="check" className="h-4 w-4" /> {L("Saqlandi", "Сохранено", "Saved", "Gespeichert")}
            </span>
          )}
        </div>
      </div>
