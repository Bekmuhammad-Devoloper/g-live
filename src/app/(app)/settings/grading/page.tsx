...
export default async function GradingSettingsPage() {
  ...
  if (!ALLOWED.includes(s.role as never)) {
    return <Forbidden title={tr(s.locale, { uz: "Kirish taqiqlangan", ru: "Доступ запрещён", en: "Access denied", de: "Zugriff verweigert" })} body={tr(s.locale, { uz: "Sozlamalar faqat direktor va administrator uchun.", ru: "Настройки доступны только директору и администратору.", en: "Settings are available only to the director and administrator.", de: "Die Einstellungen stehen nur dem Direktor und dem Administrator zur Verfügung." })} />;
  }

  return (
    <div>
      <h1 className="mb-6 text-3xl font-bold tracking-tight text-slate-900 dark:text-slate-100">{tr(s.locale, { uz: "Baholash", ru: "Оценивание", en: "Grading", de: "Bewertung" })}</h1>
