...
const SMS_TYPES: SmsType[] = [
  { key: "prepay", label: { uz: "Oldindan to'lov haqida xabarnoma", ru: "Уведомление о предоплате", en: "Prepayment notification" },
    desc: { uz: "Xabar talabaga to'lov muddati tugashidan 3 kun oldin ertalab soat 9 da yuboriladi.", ru: "Сообщение отправляется ученику за 3 дня до окончания срока оплаты, утром в 9 часов.", en: "The message is sent to the student 3 days before the payment deadline, at 9 a.m." },
    text: { uz: "Assalomu Alaykum, (STUDENT)! (LC) o'quv markazida o'qish uchun to'lovingiz tez orada tugaydi. Iltimos, oldindan o'qish uchun pul to'lang", ru: "Здравствуйте, (STUDENT)! Срок оплаты за обучение в учебном центре (LC) скоро истекает. Пожалуйста, внесите оплату заранее.", en: "Hello, (STUDENT)! Your tuition payment at (LC) is about to expire. Please pay in advance." } },
  { key: "low_balance", label: { uz: "Balans yetarli emas", ru: "Недостаточно средств на балансе", en: "Insufficient balance" }, counter: true,
    desc: { uz: "Talaba balansi belgilangan kunlik chegaradan past bo'lganda yuboriladi. Sonlagichda necha kun oldin ogohlantirish belgilanadi.", ru: "Отправляется, когда баланс ученика ниже установленного дневного порога. Счётчик задаёт, за сколько дней предупреждать.", en: "Sent when the student's balance is below the set daily threshold. The counter sets how many days in advance to warn." },
    text: { uz: "Assalomu Alaykum, (STUDENT)! (LC) markazidagi balansingiz (BALANCE) so'm. Iltimos, hisobingizni to'ldiring.", ru: "Здравствуйте, (STUDENT)! Ваш баланс в центре (LC) — (BALANCE) сум. Пожалуйста, пополните счёт.", en: "Hello, (STUDENT)! Your balance at (LC) is (BALANCE) so'm. Please top up your account." } },
  { key: "payment_done", label: { uz: "To'lov amalga oshirildi", ru: "Платёж выполнен", en: "Payment completed" },
    desc: { uz: "Talaba to'lov qilgach darhol yuboriladi.", ru: "Отправляется сразу после оплаты учеником.", en: "Sent immediately after the student makes a payment." },
    text: { uz: "Assalomu Alaykum, (STUDENT)! (SUM) so'm to'lovingiz qabul qilindi. Rahmat! (LC)", ru: "Здравствуйте, (STUDENT)! Ваш платёж на (SUM) сум принят. Спасибо! (LC)", en: "Hello, (STUDENT)! Your payment of (SUM) so'm has been received. Thank you! (LC)" } },
  { key: "student_added", label: { uz: "Talaba guruhga qo'shildi", ru: "Ученик добавлен в группу", en: "Student added to group" },
    desc: { uz: "Talaba guruhga biriktirilganda yuboriladi.", ru: "Отправляется, когда ученика прикрепляют к группе.", en: "Sent when the student is assigned to a group." },
    text: { uz: "Assalomu Alaykum, (STUDENT)! Siz (GROUP) guruhiga qo'shildingiz. Dars: (DAYS) (HOURS), (ROOM). (LC)", ru: "Здравствуйте, (STUDENT)! Вы добавлены в группу (GROUP). Занятие: (DAYS) (HOURS), (ROOM). (LC)", en: "Hello, (STUDENT)! You have been added to group (GROUP). Lesson: (DAYS) (HOURS), (ROOM). (LC)" } },
  { key: "birthday", label: { uz: "Talaba tug'ilgan kuni", ru: "День рождения ученика", en: "Student's birthday" },
    desc: { uz: "Talabaning tug'ilgan kunida ertalab yuboriladi.", ru: "Отправляется утром в день рождения ученика.", en: "Sent in the morning on the student's birthday." },
    text: { uz: "Assalomu Alaykum, (STUDENT)! Tug'ilgan kuningiz muborak bo'lsin! (LC) jamoasi", ru: "Здравствуйте, (STUDENT)! Поздравляем с днём рождения! Команда (LC)", en: "Hello, (STUDENT)! Happy birthday! The (LC) team" } },
  { key: "absent", label: { uz: "Talaba darsda ishtrok etmadi", ru: "Ученик не был на уроке", en: "Student missed the lesson" },
    desc: { uz: "Talaba darsga kelmaganda (davomat belgilangach) ota-onaga yuboriladi.", ru: "Отправляется родителям, когда ученик не пришёл на урок (после отметки посещаемости).", en: "Sent to parents when the student misses a lesson (after attendance is marked)." },
    text: { uz: "Assalomu Alaykum! Farzandingiz (STUDENT) bugun (GROUP) darsiga qatnashmadi. (LC)", ru: "Здравствуйте! Ваш ребёнок (STUDENT) сегодня не был на уроке группы (GROUP). (LC)", en: "Hello! Your child (STUDENT) did not attend the (GROUP) lesson today. (LC)" } },
];

const VARS: [string, L][] = [
  ["(STUDENT)", { uz: "Talabaning ismi", ru: "Имя ученика", en: "Student's name" }],
  ["(GROUP)", { uz: "Guruh nomi", ru: "Название группы", en: "Group name" }],
  ["(SUM)", { uz: "To'lov miqdori", ru: "Сумма платежа", en: "Payment amount" }],
  ["(LC)", { uz: "O'quv markazingiz nomi", ru: "Название вашего учебного центра", en: "Your learning center name" }],
  ["(TEACHER)", { uz: "O'qituvchini ismi", ru: "Имя преподавателя", en: "Teacher's name" }],
  ["(TIME)", { uz: "Vaqt", ru: "Время", en: "Time" }],
  ["(ROOM)", { uz: "Xona", ru: "Кабинет", en: "Room" }],
  ["(DAYS)", { uz: "Kunlar", ru: "Дни", en: "Days" }],
  ["(BALANCE)", { uz: "Talabaning hozirgi balansi", ru: "Текущий баланс ученика", en: "Student's current balance" }],
  ["(EX-ID)", { uz: "Talabaning qo'shimcha ID si", ru: "Дополнительный ID ученика", en: "Student's additional ID" }],
  ["(HOURS)", { uz: "Guruhda dars boshlanish va tugash soatlari", ru: "Часы начала и окончания занятий в группе", en: "Group lesson start and end hours" }],
  ["(COURSE)", { uz: "Talaba o'qiyotgan guruh kursi nomi", ru: "Название курса группы, где учится ученик", en: "Name of the course the student's group studies" }],
  ["(INDEBTEDNESS)", { uz: "Talabaning qarzdorligi", ru: "Задолженность ученика", en: "Student's debt" }],
  ["(GROUP INFORMATION)", { uz: "Guruh haqida ma'lumot", ru: "Информация о группе", en: "Group information" }],
];
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-center gap-1 border-b border-slate-200/70 dark:border-slate-800">
        {([["auto", "Auto-SMS"], ["templates", tr(locale, { uz: "SMS shablonlar", ru: "SMS-шаблоны", en: "SMS templates" })]] as const).map(([k, lb]) => (
      ...
      {tab === "templates" ? (
            ...
            <div key={t.key} className="rounded-2xl border border-slate-200/70 bg-white p-4 shadow-card dark:border-slate-800 dark:bg-slate-900">
              <div className="mb-1.5 flex items-center justify-between">
                ...
                <span className={cn("rounded-full px-2 py-0.5 text-[10px] font-semibold", state[t.key].enabled ? "bg-emerald-50 text-emerald-600 dark:bg-emerald-500/15 dark:text-emerald-400" : "bg-slate-100 text-slate-400 dark:bg-slate-800")}>
                  {state[t.key].enabled ? tr(locale, { uz: "Yoqilgan", ru: "Включено", en: "Enabled" }) : tr(locale, { uz: "O'chirilgan", ru: "Отключено", en: "Disabled" })}
                </span>
              </div>
        ...
        <div className="grid gap-4 lg:grid-cols-[1fr_1.2fr_1fr]">
          {/* Chap: SMS turi */}
          <div>
            <h3 className="mb-3 text-base font-semibold text-slate-700 dark:text-slate-200">{tr(locale, { uz: "SMS turi", ru: "Тип SMS", en: "SMS type" })}</h3>
          ...
          <div className="rounded-2xl border border-slate-200/70 bg-white p-5 shadow-card dark:border-slate-800 dark:bg-slate-900">
            <h3 className="mb-3 text-base font-semibold text-slate-700 dark:text-slate-200">{tr(locale, { uz: "SMS matn", ru: "Текст SMS", en: "SMS text" })}: {tr(locale, type.label)}</h3>
            <textarea
              ref={areaRef}
              value={cur.text}
            ...
            <h4 className="mb-2 mt-5 text-base font-semibold text-slate-700 dark:text-slate-200">{tr(locale, { uz: "Yuborilgan SMS misoli", ru: "Пример отправленного SMS", en: "Example of the sent SMS" })}</h4>
            ...
            <div className="mt-3 flex items-center justify-between">
              <span className="text-xs text-slate-400">{tr(locale, { uz: `${len} ta belgi (~ ${smsCount} SMS)`, ru: `${len} символов (~ ${smsCount} SMS)`, en: `${len} characters (~ ${smsCount} SMS)` })}</span>
          ...
          <div className="rounded-2xl border border-slate-200/70 bg-white p-5 shadow-card dark:border-slate-800 dark:bg-slate-900">
            <h3 className="mb-3 text-base font-semibold text-slate-700 dark:text-slate-200">{tr(locale, { uz: "Tavsif", ru: "Описание", en: "Description" })}</h3>
            ...
            <h4 className="mb-2 mt-5 text-base font-semibold text-slate-700 dark:text-slate-200">{tr(locale, { uz: "Mavjud o'zgaruvchilar:", ru: "Доступные переменные:", en: "Available variables:" })}</h4>
