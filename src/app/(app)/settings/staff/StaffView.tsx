  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        <h1 className="text-[26px] font-bold tracking-tight text-slate-900 dark:text-slate-100">{tr(locale, { uz: "Xodimlar", ru: "Сотрудники", en: "Staff", de: "Mitarbeiter" })}</h1>
        <div className="flex items-center gap-2.5">
          <button onClick={() => setDrawer({ edit: null })} className="flex h-11 items-center gap-2 rounded-full bg-brand-800 px-6 text-sm font-semibold text-white shadow-sm transition hover:bg-brand-900">
            {tr(locale, { uz: "Yangisini qo'shish", ru: "Добавить нового", en: "Add new", de: "Neu hinzufügen" })}
          </button>
          <button onClick={() => setImportOpen(true)} className="flex h-11 items-center gap-2 rounded-full border border-slate-300 bg-white px-5 text-sm font-semibold text-slate-600 transition hover:bg-slate-50 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-300 dark:hover:bg-slate-800">
            <Icon name="download" className="h-4 w-4" /> {tr(locale, { uz: "Import", ru: "Импорт", en: "Import", de: "Import" })}
          </button>
        </div>
      </div>
      ...
      {rows.length > 6 && (
        <div className="relative max-w-xs">
          ...
          <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder={tr(locale, { uz: "Xodim qidirish...", ru: "Поиск сотрудника...", en: "Search staff...", de: "Mitarbeiter suchen..." })} className="h-10 w-full rounded-lg border border-slate-200 bg-white pl-9 pr-3 text-sm outline-none focus:border-brand-400 dark:border-slate-700 dark:bg-slate-800 dark:text-slate-100" />
        </div>
      )}
        ...
        <div className="overflow-x-auto">
          <table className="w-full min-w-[820px] text-left text-sm">
            <thead className="border-b border-slate-200/70 text-[13px] font-semibold text-slate-500 dark:border-slate-800">
              <tr>
                <th className="px-6 py-4">{tr(locale, { uz: "id", ru: "id", en: "id", de: "id" })}</th>
                <th className="px-6 py-4">{tr(locale, { uz: "Ism", ru: "Имя", en: "Name", de: "Name" })}</th>
                <th className="px-6 py-4">{tr(locale, { uz: "Roli", ru: "Роль", en: "Role", de: "Rolle" })}</th>
                <th className="px-6 py-4">{tr(locale, { uz: "Lavozimi", ru: "Должность", en: "Position", de: "Position" })}</th>
                <th className="px-6 py-4">{tr(locale, { uz: "Telefon", ru: "Телефон", en: "Phone", de: "Telefon" })}</th>
                <th className="px-6 py-4 text-right">{tr(locale, { uz: "Amallar", ru: "Действия", en: "Actions", de: "Aktionen" })}</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
              {shown.length === 0 ? (
                <tr><td colSpan={6} className="py-16 text-center text-slate-400">{tr(locale, { uz: "Xodim topilmadi", ru: "Сотрудник не найден", en: "No staff found", de: "Kein Mitarbeiter gefunden" })}</td></tr>
              ) : shown.map((r) => (
                <tr key={r.id} className="group transition hover:bg-slate-50 dark:hover:bg-slate-800/40">
                  ...
                  <td className="px-6 py-4">
                    <div className="flex items-center justify-end gap-2">
                      <button onClick={() => onInvite(r.id)} title={tr(locale, { uz: "Taklif / xabar yuborish", ru: "Отправить приглашение / сообщение", en: "Send invite / message", de: "Einladung / Nachricht senden" })} className="grid h-8 w-8 place-items-center rounded-lg text-amber-500 transition hover:bg-amber-50 dark:hover:bg-amber-500/10"><Icon name="mail" className="h-5 w-5" /></button>
                      <button onClick={() => setDrawer({ edit: r })} title={tr(locale, { uz: "Tahrirlash", ru: "Редактировать", en: "Edit", de: "Bearbeiten" })} className="grid h-8 w-8 place-items-center rounded-lg text-slate-400 opacity-0 transition hover:bg-slate-100 hover:text-brand-600 group-hover:opacity-100 dark:hover:bg-slate-800"><Icon name="pencil" className="h-4 w-4" /></button>
                      {/* O'chirish — faqat direktor va o'rinbosarida */}
                      {canDelete && <button onClick={() => onDelete(r)} title={tr(locale, { uz: "Olib tashlash", ru: "Удалить", en: "Remove", de: "Entfernen" })} className="grid h-8 w-8 place-items-center rounded-lg text-slate-400 opacity-0 transition hover:bg-rose-50 hover:text-rose-600 group-hover:opacity-100 dark:hover:bg-rose-500/10"><Icon name="trash" className="h-4 w-4" /></button>}
                    </div>
                  </td>
                </tr>
              ))}
...
function StaffDrawer({ locale, edit, roleOptions, sipOptions, onClose, onSaved }: { locale: Locale; edit: VStaff | null; roleOptions: RoleOption[]; sipOptions: SipOption[]; onClose: () => void; onSaved: (msg: string) => void }) {
  ...
  const submit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    start(async () => {
      ...
      if (r.ok) onSaved(edit ? tr(locale, { uz: "Saqlandi", ru: "Сохранено", en: "Saved", de: "Gespeichert" }) : tr(locale, { uz: "Xodim qo'shildi", ru: "Сотрудник добавлен", en: "Staff member added", de: "Mitarbeiter hinzugefügt" }));
  ...
  return createPortal(
    <div className="fixed inset-0 z-[80]" onMouseDown={onClose}>
      ...
      <form ref={formRef} onSubmit={submit} onMouseDown={(e) => e.stopPropagation()} className="animate-slide-in-right absolute right-0 top-0 flex h-full drawer-panel flex-col border-l border-slate-200 bg-white shadow-pop dark:border-white/10 dark:bg-[#15243d]">
        ...
        <div className="flex shrink-0 items-center justify-between border-b border-slate-100 px-5 py-4 dark:border-white/10">
          <h3 className="text-base font-bold text-slate-800 dark:text-slate-100">{edit ? tr(locale, { uz: "Xodimni tahrirlash", ru: "Редактировать сотрудника", en: "Edit staff member", de: "Mitarbeiter bearbeiten" }) : tr(locale, { uz: "Yangi xodim qo'shish", ru: "Добавить нового сотрудника", en: "Add new staff member", de: "Neuen Mitarbeiter hinzufügen" })}</h3>
        ...
        <div className="flex-1 space-y-3.5 overflow-y-auto px-5 py-4">
          <Field label={tr(locale, { uz: "F.I.Sh.", ru: "Ф.И.О.", en: "Full name", de: "Vollständiger Name" })} req><input name="fullName" required defaultValue={edit?.fullName ?? ""} placeholder={tr(locale, { uz: "Ism Familiya", ru: "Имя Фамилия", en: "First Last", de: "Vorname Nachname" })} className={inp} autoFocus /></Field>
          {!edit && <Field label={tr(locale, { uz: "Email (login)", ru: "Email (логин)", en: "Email (login)", de: "E-Mail (Login)" })} req><input name="email" type="email" required placeholder="xodim@gl.uz" className={inp} /></Field>}
          <div className="grid grid-cols-2 gap-3">
            <Field label={tr(locale, { uz: "Rol", ru: "Роль", en: "Role", de: "Rolle" })} req>
        ...
        <div className="flex shrink-0 gap-2 border-t border-slate-100 px-5 py-4 dark:border-white/10">
          <button type="button" onClick={onClose} className="flex-1 rounded-xl border border-slate-200 py-2.5 text-sm font-semibold text-slate-600 hover:bg-slate-50 dark:border-white/10 dark:text-slate-300 dark:hover:bg-white/5">{tr(locale, { uz: "Bekor qilish", ru: "Отмена", en: "Cancel", de: "Abbrechen" })}</button>
          <button type="submit" disabled={pending} className="flex-[1.4] rounded-xl bg-brand-600 py-2.5 text-sm font-semibold text-white hover:bg-brand-700 disabled:opacity-60">{pending ? tr(locale, { uz: "Saqlanmoqda...", ru: "Сохранение...", en: "Saving...", de: "Wird gespeichert..." }) : tr(locale, { uz: "Saqlash", ru: "Сохранить", en: "Save", de: "Speichern" })}</button>
        </div>
      </form>
    </div>,
    document.body,
...
function ImportModal({ locale, onClose, onDone }: { locale: Locale; onClose: () => void; onDone: (msg: string) => void }) {
  ...
  const run = () => {
    ...
    if (rows.length === 0) { setError(tr(locale, { uz: "Ma'lumot topilmadi. CSV: fullName,email,phone,position,role,password", ru: "Данные не найдены. CSV: fullName,email,phone,position,role,password", en: "No data found. CSV: fullName,email,phone,position,role,password", de: "Keine Daten gefunden. CSV: fullName,email,phone,position,role,password" })); return; }
    start(async () => {
      ...
      onDone(tr(locale, {
        ...
        de: `${res.created} hinzugefügt${res.skipped ? `, ${res.skipped} übersprungen` : ""}`,
      }));
    });
  };
    ...
    <div className="fixed inset-0 z-[85] flex items-center justify-center bg-slate-900/50 p-4 backdrop-blur-sm" onMouseDown={onClose}>
      <div className="w-full max-w-lg rounded-2xl border border-slate-200 bg-white shadow-pop dark:border-slate-800 dark:bg-slate-900" onMouseDown={(e) => e.stopPropagation()}>
        <div className="flex items-center justify-between border-b border-slate-100 px-5 py-4 dark:border-slate-800">
          <h3 className="text-base font-bold text-slate-800 dark:text-slate-100">{tr(locale, { uz: "Xodimlarni import qilish", ru: "Импорт сотрудников", en: "Import staff", de: "Mitarbeiter importieren" })}</h3>
          <button onClick={onClose} className="text-slate-400 hover:text-slate-600"><Icon name="close" className="h-5 w-5" /></button>
        </div>
        <div className="space-y-3 px-5 py-4">
          <p className="text-xs text-slate-500 dark:text-slate-400">{tr(locale, { uz: "CSV ustunlari:", ru: "Столбцы CSV:", en: "CSV columns:", de: "CSV-Spalten:" })} <code className="rounded bg-slate-100 px-1 dark:bg-slate-800">fullName, email, phone, position, role, password</code>. {tr(locale, { uz: "Rol: MANAGER/TEACHER/ADMIN/DIRECTOR/DEPUTY_DIRECTOR. Parol bo'lmasa", ru: "Роль: MANAGER/TEACHER/ADMIN/DIRECTOR/DEPUTY_DIRECTOR. Если пароль не указан,", en: "Role: MANAGER/TEACHER/ADMIN/DIRECTOR/DEPUTY_DIRECTOR. If no password is given,", de: "Rolle: MANAGER/TEACHER/ADMIN/DIRECTOR/DEPUTY_DIRECTOR. Falls kein Passwort angegeben wird," })} <b>12345678</b> {tr(locale, { uz: "beriladi.", ru: "назначается.", en: "is assigned.", de: "wird zugewiesen." })}</p>
          <label className="flex h-11 cursor-pointer items-center justify-center gap-2 rounded-lg border border-dashed border-slate-300 text-sm font-medium text-slate-500 hover:bg-slate-50 dark:border-slate-700 dark:hover:bg-slate-800">
            <Icon name="download" className="h-4 w-4" /> {tr(locale, { uz: "CSV fayl tanlash", ru: "Выбрать CSV-файл", en: "Choose CSV file", de: "CSV-Datei auswählen" })}
            <input type="file" accept=".csv,text/csv,text/plain" className="hidden" onChange={(e) => onFile(e.target.files?.[0])} />
          </label>
          <div className="text-center text-[11px] text-slate-400">{tr(locale, { uz: "yoki jadvalni shu yerga joylang", ru: "или вставьте таблицу сюда", en: "or paste the table here", de: "oder Tabelle hier einfügen" })}</div>
          <textarea value={text} onChange={(e) => setText(e.target.value)} rows={6} placeholder={tr(locale, { uz: "Ism Familiya,email@gl.uz,+99890...,CEO,DIRECTOR,parol", ru: "Имя Фамилия,email@gl.uz,+99890...,CEO,DIRECTOR,пароль", en: "First Last,email@gl.uz,+99890...,CEO,DIRECTOR,password", de: "Vorname Nachname,email@gl.uz,+99890...,CEO,DIRECTOR,passwort" })} className="w-full rounded-lg border border-slate-200 bg-white px-3 py-2 font-mono text-xs outline-none focus:border-brand-400 dark:border-slate-700 dark:bg-slate-800/60 dark:text-slate-100" />
          {error && <div className="rounded-lg bg-rose-50 px-3 py-2 text-sm text-rose-600 dark:bg-rose-500/10 dark:text-rose-400">{error}</div>}
        </div>
        <div className="flex gap-2 border-t border-slate-100 px-5 py-4 dark:border-slate-800">
          <button onClick={onClose} className="flex-1 rounded-xl border border-slate-200 py-2.5 text-sm font-semibold text-slate-600 hover:bg-slate-50 dark:border-slate-700 dark:text-slate-300 dark:hover:bg-slate-800">{tr(locale, { uz: "Bekor qilish", ru: "Отмена", en: "Cancel", de: "Abbrechen" })}</button>
          <button onClick={run} disabled={pending} className="flex-[1.4] rounded-xl bg-brand-600 py-2.5 text-sm font-semibold text-white hover:bg-brand-700 disabled:opacity-60">{pending ? tr(locale, { uz: "Import qilinmoqda...", ru: "Импорт...", en: "Importing...", de: "Wird importiert..." }) : tr(locale, { uz: "Import qilish", ru: "Импортировать", en: "Import", de: "Importieren" })}</button>
        </div>
      </div>
    </div>,
    document.body,
