      ...
      >
        ...
        <div className="flex shrink-0 items-center justify-between border-b border-slate-100 px-5 py-4 dark:border-white/10">
          <div className="flex items-center gap-2 text-base font-bold text-slate-800 dark:text-slate-100">
            ...
            {tr(locale, { uz: "Yangi o'qituvchi qo'shish", ru: "Добавить преподавателя", en: "Add teacher", de: "Lehrer hinzufügen" })}
          </div>
        ...
        <div className="flex-1 space-y-3 overflow-y-auto px-5 py-4">
          <Field label={tr(locale, { uz: "Ism-familiya *", ru: "Имя-фамилия *", en: "Full name *", de: "Vor- und Nachname *" })} value={fullName} onChange={setFullName} placeholder={tr(locale, { uz: "Masalan: Nigora Rashidova", ru: "Например: Нигора Рашидова", en: "e.g. Nigora Rashidova", de: "z. B. Nigora Raschidova" })} autoFocus />

          {/* Jinsi */}
          <div>
            <span className="mb-1 block text-[11px] font-medium text-slate-400">{tr(locale, { uz: "Jinsi", ru: "Пол", en: "Gender", de: "Geschlecht" })}</span>
            <div className="grid grid-cols-2 gap-2">
              <GenderBtn active={gender === "MALE"} onClick={() => setGender("MALE")} icon="♂" label={tr(locale, { uz: "Erkak", ru: "Мужчина", en: "Male", de: "Männlich" })} color="#3b82f6" />
              <GenderBtn active={gender === "FEMALE"} onClick={() => setGender("FEMALE")} icon="♀" label={tr(locale, { uz: "Ayol", ru: "Женщина", en: "Female", de: "Weiblich" })} color="#ec4899" />
            </div>
          </div>

          <div className="grid grid-cols-2 gap-3">
            <div>
              <span className="mb-1 block text-[11px] font-medium text-slate-400">{tr(locale, { uz: "Telefon", ru: "Телефон", en: "Phone", de: "Telefon" })}</span>
            ...
            <div>
              <span className="mb-1 block text-[11px] font-medium text-slate-400">{tr(locale, { uz: "Filial", ru: "Филиал", en: "Branch", de: "Filiale" })}</span>
              <select
                value={branchId}
              ...
              >
                <option value="">{tr(locale, { uz: "— tanlanmagan —", ru: "— не выбрано —", en: "— not selected —", de: "— nicht ausgewählt —" })}</option>
                {branches.map((b) => <option key={b.id} value={b.id}>{b.name}</option>)}
              </select>
            </div>
          </div>
          <Field label={tr(locale, { uz: "Email * (login uchun)", ru: "Email * (для входа)", en: "Email * (for login)", de: "E-Mail * (für Login)" })} value={email} onChange={setEmail} placeholder="teacher@gl.uz" type="email" />
          <Field label={tr(locale, { uz: "Parol *", ru: "Пароль *", en: "Password *", de: "Passwort *" })} value={password} onChange={setPassword} placeholder={tr(locale, { uz: "Kamida 4 ta belgi", ru: "Минимум 4 символа", en: "At least 4 characters", de: "Mindestens 4 Zeichen" })} type="text" />
          <div>
            <span className="mb-1 block text-[11px] font-medium text-slate-400">{tr(locale, { uz: "Fiksa (belgilangan oylik maosh)", ru: "Фикса (фиксированная зарплата)", en: "Base (fixed monthly salary)", de: "Fixgehalt (festes Monatsgehalt)" })}</span>
            <div className="relative">
              ...
              />
              <span className="pointer-events-none absolute right-3 top-1/2 -translate-y-1/2 text-xs font-medium text-slate-400">{tr(locale, { uz: "so'm", ru: "сум", en: "so'm", de: "UZS" })}</span>
            </div>
          </div>

          {/* KPI bonusi */}
          <div>
            <span className="mb-1 block text-[11px] font-medium text-slate-400">{tr(locale, { uz: "KPI bonusi (so'm)", ru: "KPI бонус (сум)", en: "KPI bonus (so'm)", de: "KPI-Bonus (UZS)" })}</span>
            <div className="relative">
              <input
                type="text"
                inputMode="numeric"
        ...
        <div className="flex shrink-0 gap-2 border-t border-slate-100 px-5 py-4 dark:border-white/10">
          ...
          >
            {pending ? tr(locale, { uz: "Qo'shilmoqda...", ru: "Добавление...", en: "Adding...", de: "Wird hinzugefügt..." }) : tr(locale, { uz: "Qo'shish", ru: "Добавить", en: "Add", de: "Hinzufügen" })}
          </button>
        </div>
      </div>
    </div>,
