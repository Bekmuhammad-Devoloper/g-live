      ...
      >
        ...
        <div className="flex items-center justify-between border-b border-slate-100 px-5 py-4 dark:border-white/10">
          <div className="flex min-w-0 items-center gap-3">
            ...
            <div className="min-w-0">
              ...
              <p className="mt-0.5 flex items-center gap-1 text-xs text-slate-400"><Icon name="wallet" className="h-3.5 w-3.5 text-amber-500" /> {tr(locale, { uz: "Maosh boshqaruvi", ru: "Управление зарплатой", en: "Salary management" })} · {monthLabel(new Date().getFullYear(), new Date().getMonth() + 1, locale)}</p>
            </div>
          </div>
        ...
        <div className="px-5 py-4">
          <div className="mb-3 flex items-center justify-between">
            <span className="text-xs font-semibold uppercase tracking-wide text-slate-400">{tr(locale, { uz: "Joriy oy", ru: "Текущий месяц", en: "Current month" })}</span>
            <span className="rounded-full bg-emerald-500/15 px-2 py-0.5 text-[11px] font-semibold text-emerald-600 dark:text-emerald-400">{tr(locale, { uz: "Ochiq", ru: "Открыт", en: "Open" })}</span>
          </div>

          <div className="grid grid-cols-2 gap-3">
            <Field label={tr(locale, { uz: "Fiksa (asosiy)", ru: "Фикса (основная)", en: "Base (fixed)" })} value={fiksa} onChange={setFiksa} disabled={!canManage} color="#3b82f6" />
            <Field label={tr(locale, { uz: "Bonus", ru: "Бонус", en: "Bonus" })} value={bonus} onChange={setBonus} disabled={!canManage} color="#10b981" />
            <Field label={tr(locale, { uz: "Jarima", ru: "Штраф", en: "Penalty" })} value={penalty} onChange={setPenalty} disabled={!canManage} color="#ef4444" />
            <Field label={tr(locale, { uz: "KPI bonus (so'm)", ru: "KPI бонус (сум)", en: "KPI bonus (so'm)" })} value={kpi} onChange={setKpi} disabled={!canManage} color="#8b5cf6" />
          </div>

          {/* Jami */}
          <div className="mt-4 flex items-center justify-between rounded-xl bg-slate-50 px-4 py-3 dark:bg-white/[0.04]">
            <span className="text-sm font-medium text-slate-500 dark:text-slate-300">{tr(locale, { uz: "Bu oy jami", ru: "Итого за месяц", en: "Total this month" })}</span>
        ...
        <div className="border-t border-slate-100 px-5 py-4 dark:border-white/10">
          <div className="mb-2 flex items-center gap-2 text-xs font-semibold uppercase tracking-wide text-slate-400">
            <Icon name="history" className="h-3.5 w-3.5" /> {tr(locale, { uz: "Oldingi oylar", ru: "Прошлые месяцы", en: "Previous months" })}
          </div>
          {t.history.length === 0 ? (
            <p className="py-3 text-center text-xs text-slate-400">{tr(locale, { uz: "Tarix hali yo'q", ru: "Истории пока нет", en: "No history yet" })}</p>
          ) : (
            <div className="overflow-x-auto rounded-xl border border-slate-100 dark:border-white/10">
              <table className="w-full min-w-[420px] text-left text-xs">
                <thead className="bg-slate-50 text-[10px] uppercase tracking-wide text-slate-400 dark:bg-white/[0.03]">
                  <tr>
                    <th className="px-3 py-2">{tr(locale, { uz: "Oy", ru: "Месяц", en: "Month" })}</th>
                    <th className="px-3 py-2 text-right">{tr(locale, { uz: "Fiksa", ru: "Фикса", en: "Base" })}</th>
                    <th className="px-3 py-2 text-right">{tr(locale, { uz: "Bonus", ru: "Бонус", en: "Bonus" })}</th>
                    <th className="px-3 py-2 text-right">{tr(locale, { uz: "Jarima", ru: "Штраф", en: "Penalty" })}</th>
                    <th className="px-3 py-2 text-right">KPI</th>
                    <th className="px-3 py-2 text-right">{tr(locale, { uz: "Jami", ru: "Итого", en: "Total" })}</th>
                  </tr>
                </thead>
              ...
              <p className="px-3 py-1.5 text-right text-[10px] text-slate-400">{tr(locale, { uz: "summalar so'mda", ru: "суммы в сумах", en: "amounts in so'm" })}</p>
            </div>
          )}
        </div>
      </div>
...
function CredentialsSection({ teacherId, email, locale }: { teacherId: string; email: string | null; locale: Locale }) {
  ...
  const savePw = () => startSave(async () => {
    ...
    if (r.ok) { setPassword(newPw); setShow(true); setNewPw(""); setMsg(tr(locale, { uz: "Parol yangilandi", ru: "Пароль обновлён", en: "Password updated" })); }
  ...
  return (
    <div className="border-t border-slate-100 px-5 py-4 dark:border-white/10">
      <div className="mb-2 flex items-center gap-2 text-xs font-semibold uppercase tracking-wide text-slate-400">
        <Icon name="key" className="h-3.5 w-3.5" /> {tr(locale, { uz: "Kirish ma'lumotlari", ru: "Данные для входа", en: "Login credentials" })}
      </div>
      <div className="space-y-2">
        ...
        <div className="flex items-center gap-2">
          ...
          {email && <button type="button" onClick={() => copy(email)} className={ico} title={tr(locale, { uz: "Nusxalash", ru: "Копировать", en: "Copy" })}><Icon name="copy" className="h-4 w-4" /></button>}
        </div>
        {/* Parol */}
        <div className="flex items-center gap-2">
          <span className="w-12 shrink-0 text-xs text-slate-400">{tr(locale, { uz: "Parol", ru: "Пароль", en: "Password" })}</span>
          <div className={box + " font-mono"}>
            {password === undefined ? "••••••••" : show ? (password || tr(locale, { uz: "— (o'rnatilmagan, yangi parol qo'ying)", ru: "— (не задан)", en: "— (not set)" })) : "••••••••"}
          </div>
          {password === undefined ? (
            <button type="button" onClick={reveal} disabled={loading} className={ico} title={tr(locale, { uz: "Ko'rsatish", ru: "Показать", en: "Show" })}><Icon name={loading ? "refresh" : "eye"} className={`h-4 w-4 ${loading ? "animate-spin" : ""}`} /></button>
          ) : (
            <>
              <button type="button" onClick={() => setShow((v) => !v)} className={ico} title={tr(locale, { uz: "Ko'rsatish/yashirish", ru: "Показать/скрыть", en: "Show/hide" })}><Icon name="eye" className="h-4 w-4" /></button>
              {show && password && <button type="button" onClick={() => copy(password)} className={ico} title={tr(locale, { uz: "Nusxalash", ru: "Копировать", en: "Copy" })}><Icon name="copy" className="h-4 w-4" /></button>}
            </>
          )}
        </div>
        ...
        <div className="flex items-center gap-2 pt-1">
          ...
          <button type="button" onClick={savePw} disabled={saving || newPw.trim().length < 4} className="h-9 shrink-0 rounded-lg bg-brand-600 px-3 text-xs font-semibold text-white transition hover:bg-brand-700 disabled:opacity-60">{saving ? "…" : tr(locale, { uz: "O'rnatish", ru: "Задать", en: "Set" })}</button>
        </div>
