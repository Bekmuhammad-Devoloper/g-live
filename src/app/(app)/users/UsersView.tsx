...
export default function UsersView({ staff, positions, branches, canManage, locale }: {
  ...
  const exportCsvNow = () => {
    exportRows(
      tr(locale, { uz: "xodimlar", ru: "сотрудники", en: "staff" }),
      [
        { key: "name", label: tr(locale, { uz: "To'liq nomi", ru: "Полное имя", en: "Full name" }) },
        { key: "gender", label: tr(locale, { uz: "Jinsi", ru: "Пол", en: "Gender" }) },
        { key: "students", label: tr(locale, { uz: "Aktiv o'quvchilar", ru: "Активные ученики", en: "Active students" }) },
        { key: "role", label: tr(locale, { uz: "Turi", ru: "Тип", en: "Type" }) },
        { key: "branch", label: tr(locale, { uz: "Filial", ru: "Филиал", en: "Branch" }) },
        { key: "phone", label: tr(locale, { uz: "Telefon", ru: "Телефон", en: "Phone" }) },
        { key: "courses", label: tr(locale, { uz: "Kurs", ru: "Курс", en: "Course" }) },
        { key: "status", label: tr(locale, { uz: "Holat", ru: "Статус", en: "Status" }) },
      ],
      filtered.map((u) => ({
        name: u.name,
        gender: u.gender === "MALE" ? tr(locale, { uz: "Erkak", ru: "Мужской", en: "Male" }) : u.gender === "FEMALE" ? tr(locale, { uz: "Ayol", ru: "Женский", en: "Female" }) : "",
        students: u.students,
        role: u.roleLabel,
        branch: u.branch ?? "",
        phone: u.phone ?? "",
        courses: u.courses.join(", "),
        status: u.active ? tr(locale, { uz: "Faol", ru: "Активен", en: "Active" }) : tr(locale, { uz: "Nofaol", ru: "Неактивен", en: "Inactive" }),
      })),
    );
  };
  ...
  return (
    <div className="space-y-4">
      <h1 className="text-[22px] font-bold tracking-tight text-slate-900 dark:text-slate-100">{tr(locale, { uz: "Xodimlar", ru: "Сотрудники", en: "Staff" })}</h1>
      ...
      <div className="flex flex-wrap items-center gap-2">
        {canManage && (
          <button onClick={() => setAddOpen(true)} className="flex h-10 items-center gap-1.5 rounded-lg bg-brand-600 px-4 text-sm font-semibold text-white shadow-sm transition hover:bg-brand-700">
            <Icon name="plus" className="h-4 w-4" /> {tr(locale, { uz: "Xodim qo'shish", ru: "Добавить сотрудника", en: "Add staff" })}
          </button>
        )}
        <div className="relative">
          <Icon name="search" className="pointer-events-none absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-slate-400" />
          <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder={tr(locale, { uz: "Qidiruv", ru: "Поиск", en: "Search" })} className="h-10 w-52 rounded-lg border border-slate-200 bg-white pl-9 pr-3 text-sm outline-none focus:border-brand-400 dark:border-slate-700 dark:bg-slate-800 dark:text-slate-100" />
        </div>
        <Select value={status} onChange={setStatus} placeholder={tr(locale, { uz: "Holat", ru: "Статус", en: "Status" })} options={[{ v: "active", label: tr(locale, { uz: "Faol", ru: "Активен", en: "Active" }) }, { v: "inactive", label: tr(locale, { uz: "Nofaol", ru: "Неактивен", en: "Inactive" }) }]} />
        <Select value={roleFilter} onChange={setRoleFilter} placeholder={tr(locale, { uz: "Vazifa", ru: "Должность", en: "Role" })} options={roleOptions.map((r) => ({ v: r, label: r }))} />
        ...
        <div className="overflow-x-auto">
          <table className="w-full min-w-[980px] text-left text-sm">
            <thead className="border-b border-slate-200/70 bg-slate-50/80 text-[11px] font-semibold uppercase tracking-wider text-slate-500 dark:border-slate-800 dark:bg-slate-800/50">
              <tr>
                <th className="w-12 px-4 py-3">№</th><th className="px-4 py-3">{tr(locale, { uz: "To'liq nomi", ru: "Полное имя", en: "Full name" })}</th><th className="px-4 py-3">{tr(locale, { uz: "Jinsi", ru: "Пол", en: "Gender" })}</th>
                <th className="px-4 py-3 text-center">{tr(locale, { uz: "Aktiv o'quvchilar soni", ru: "Кол-во активных учеников", en: "Active students count" })}</th><th className="px-4 py-3">{tr(locale, { uz: "Guruhlar", ru: "Группы", en: "Groups" })}</th><th className="px-4 py-3">{tr(locale, { uz: "Turi", ru: "Тип", en: "Type" })}</th>
                <th className="px-4 py-3">{tr(locale, { uz: "Filiallar", ru: "Филиалы", en: "Branches" })}</th><th className="px-4 py-3">{tr(locale, { uz: "Telefon raqam", ru: "Номер телефона", en: "Phone number" })}</th><th className="px-4 py-3">{tr(locale, { uz: "Kurs", ru: "Курс", en: "Course" })}</th>
                ...
                <tr key={u.id} onClick={canManage ? () => setDetailId(u.id) : undefined} className={cn("hover:bg-slate-50 dark:hover:bg-slate-800/50", canManage && "cursor-pointer")}>
                  ...
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-2 font-medium text-slate-800 dark:text-slate-100">
                      {u.name}
                      {!u.active && <span className="rounded bg-slate-200 px-1.5 py-0.5 text-[10px] font-semibold text-slate-500 dark:bg-slate-700">{tr(locale, { uz: "Nofaol", ru: "Неактивен", en: "Inactive" })}</span>}
                    </div>
                  </td>
                  <td className="whitespace-nowrap px-4 py-3">{u.gender === "MALE" ? <span style={{ color: "#3b82f6" }}>♂ {tr(locale, { uz: "Erkak", ru: "Мужской", en: "Male" })}</span> : u.gender === "FEMALE" ? <span style={{ color: "#ec4899" }}>♀ {tr(locale, { uz: "Ayol", ru: "Женский", en: "Female" })}</span> : <span className="text-slate-400">—</span>}</td>
                    ...
                    <td className="px-4 py-3">
                      <div className="flex items-center justify-end">
                        <button
                          ...
                          disabled={pending}
                          title={u.active ? tr(locale, { uz: "Nofaol qilish", ru: "Деактивировать", en: "Deactivate" }) : tr(locale, { uz: "Faollashtirish", ru: "Активировать", en: "Activate" })}
...
function StaffForm({ positions, branches, onClose, locale }: { positions: PosOpt[]; branches: Opt[]; onClose: () => void; locale: Locale }) {
  ...
  const posGroups = useMemo(() => {
    ...
    for (const p of positions) {
      const key = p.department || tr(locale, { uz: "Boshqa", ru: "Другое", en: "Other" });
  ...
  const submit = () => {
    ...
    start(async () => { const r = await createStaff(fd); if (r.ok) { onClose(); router.refresh(); } else setError(r.error ?? tr(locale, { uz: "Xatolik", ru: "Ошибка", en: "Error" })); });
  };
    ...
    <div className="fixed inset-0 z-[80]" onMouseDown={onClose}>
      ...
      <form ref={formRef} onMouseDown={(e) => e.stopPropagation()} className="animate-slide-in-right absolute right-0 top-0 flex h-full w-[560px] max-w-[95%] flex-col border-l border-slate-200 bg-white shadow-pop dark:border-white/10 dark:bg-[#15243d]">
        <div className="flex shrink-0 items-center justify-between border-b border-slate-100 px-5 py-4 dark:border-white/10">
          <h3 className="text-base font-bold text-slate-800 dark:text-slate-100">{tr(locale, { uz: "Xodim qo'shish", ru: "Добавить сотрудника", en: "Add staff" })}</h3>
        ...
        <div className="flex-1 space-y-3.5 overflow-y-auto px-5 py-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div><label className={lbl}>{tr(locale, { uz: "Ism", ru: "Имя", en: "First name" })} {req}</label><input name="ism" required placeholder={tr(locale, { uz: "Ism", ru: "Имя", en: "First name" })} className={inp} /></div>
            <div><label className={lbl}>{tr(locale, { uz: "Familiya", ru: "Фамилия", en: "Last name" })}</label><input name="familiya" placeholder={tr(locale, { uz: "Familiya", ru: "Фамилия", en: "Last name" })} className={inp} /></div>
          </div>
          {/* Telefon */}
          <div>
            <label className={lbl}>{tr(locale, { uz: "Telefon raqam", ru: "Номер телефона", en: "Phone number" })} {req}</label>
            <div className="flex">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className={lbl}>{tr(locale, { uz: "O'quv markazidagi vazifasi", ru: "Должность в учебном центре", en: "Role at the learning center" })} {req}</label>
            ...
            <div>
              <label className={lbl}>{tr(locale, { uz: "Jinsi", ru: "Пол", en: "Gender" })}</label>
              <select value={gender} onChange={(e) => setGender(e.target.value as never)} className={inp}>
                <option value="">{tr(locale, { uz: "Jinsini tanlang", ru: "Выберите пол", en: "Select gender" })}</option><option value="MALE">{tr(locale, { uz: "Erkak", ru: "Мужской", en: "Male" })}</option><option value="FEMALE">{tr(locale, { uz: "Ayol", ru: "Женский", en: "Female" })}</option>
              </select>
            </div>
          </div>
          {/* Tug'ilgan sana / Filial */}
          <div className="grid grid-cols-2 gap-3">
            <div><label className={lbl}>{tr(locale, { uz: "Tug'ilgan sanasi", ru: "Дата рождения", en: "Date of birth" })}</label><input name="birthDate" type="date" className={inp} /></div>
            <div><label className={lbl}>{tr(locale, { uz: "Filial", ru: "Филиал", en: "Branch" })}</label><select name="branchId" defaultValue="" className={inp}><option value="">{tr(locale, { uz: "Tanlang", ru: "Выберите", en: "Select" })}</option>{branches.map((b) => <option key={b.id} value={b.id}>{b.name}</option>)}</select></div>
          </div>

          {/* Ish haqi chiqarish */}
          <div className="rounded-xl border border-slate-100 bg-slate-50/60 p-3 dark:border-white/5 dark:bg-white/[0.03]">
            <Switch on={salaryOn} onToggle={() => setSalaryOn((v) => !v)} label={tr(locale, { uz: "Ish haqi chiqarish", ru: "Начислять зарплату", en: "Enable salary" })} />
            {salaryOn && (
              <div className="mt-3">
                <label className={lbl}>{tr(locale, { uz: "Ish haqi (oylik, so'm)", ru: "Зарплата (в месяц, сум)", en: "Salary (monthly, UZS)" })}</label>
          ...
          {/* Email / Parol */}
          <div className="grid grid-cols-2 gap-3">
            <div><label className={lbl}>{tr(locale, { uz: "Elektron pochta", ru: "Электронная почта", en: "Email" })} {req}</label><input name="email" type="email" required placeholder="example@gmail.com" className={inp} /></div>
            <div><label className={lbl}>{tr(locale, { uz: "Parol (login uchun)", ru: "Пароль (для входа)", en: "Password (for login)" })} {req}</label><input name="password" type="text" required placeholder={tr(locale, { uz: "Kamida 4 ta belgi", ru: "Минимум 4 символа", en: "At least 4 characters" })} className={inp} /></div>
          </div>
